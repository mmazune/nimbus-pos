...
export class AttendanceService {
        ...
        if (existing) {
            ...
            if (existing.clockOutAt) {
                throw new ConflictException(
                    `Employee "${dto.employeeId}" already clocked out for today`,
                );
            }
            const now = new Date();
    ...
    async createLeaveRequest(
        ...
        await this.audit.log({
            ...
            entityId: record.id,
            metadata: { employeeId: dto.employeeId, leaveType: dto.leaveType, orgId, branchId, ...auditMeta },
            ipAddress: auditMeta?.ipAddress,
            userAgent: auditMeta?.userAgent,
        });
    ...
    async reviewLeaveRequest(
        ...
        await this.audit.log({
            ...
            entityType: 'LeaveRequest',
            entityId: record.id,
            metadata: { previousStatus: existing.status, newStatus: dto.status, orgId, branchId, ...auditMeta },
            ipAddress: auditMeta?.ipAddress,
            userAgent: auditMeta?.userAgent,
        });
    ...
    async createShiftSwap(
        ...
        await this.audit.log({
            ...
            entityId: record.id,
            metadata: { requesterEmployeeId: dto.requesterEmployeeId, targetEmployeeId: dto.targetEmployeeId, orgId, branchId, ...auditMeta },
            ipAddress: auditMeta?.ipAddress,
            userAgent: auditMeta?.userAgent,
        });
