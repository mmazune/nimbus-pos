...
| --- | --- | --- | --- |
| **B5.1 — Customers + Vendors lists** | AR invoices + AP bills as C4 list views with C5 detail forms; supplier/customer records; the highest-leverage starting point per NG-03. | M | B0 go |
| **B5.2 — Bank reconciliation workbench** | Statements + import + the match/skip/complete reconciliation flow. The most operationally valuable accounting surface and the one with the richest existing API. | M | B5.1 |
| **B5.3 — Accounting core + Review** | Journal entries (read; **write only if B0 proves Manager holds `journals:create`/`reverse` — the guides say it does not**), posting runs, posting errors, the audit-trail rail. | M | B5.1 |
| **B5.4 — Closing** | Fiscal periods open/close/lock + period-close runs. **This is Nimbus's honest analogue of Odoo's Tax Returns + Lock Dates** (NG-17) — a **period-close + lock-dates page**, not a tax-return document. | S | B5.3 |
| **B5.5 — Reporting + Configuration** | Aged Receivable/Payable views (the **first true financial reports Nimbus can ship**), budgets vs actuals, procurement suggestions, demand calendar; then Chart of Accounts, cost centres, posting source maps, tax config. | M | B5.1 |
| **B5.6 — Accounting dashboard** | The C10 card grid over the ledgers built above; per-ledger cards with count→drill-in + amount, mixed-weight buttons, and charts **only if** a real series exists. | S | B5.1–B5.5 |
...
| B | **B5** | Accounting suite over ~90 endpoints — 7-menu tree, sub-phased B5.1…B5.6 | **L** | 🟢 **GO (2026-08-21)** + B1 (+ B3 primitives). ~~C-21~~ cleared (cutover); ~~🔴 PC-03~~ and ~~🔴 PC-04~~ **cleared by backend gap batch 2** — accounting reads are branch-scoped (proven **19 failed → 31 passed** on the same suite) and the duplicate-bill guard works (**3 bills / 450,000 → 1 bill / 150,000** on three clicks). **B5.1 and B5.2 are 🟢 Ready.** Two 🟡 **design decisions** remain, not defects: **PC-06** (10 bare-array lists with no server `total` — ship as explicitly unpaginated or gain an envelope first; **never fabricate a total from `array.length`**) and **PC-01/PC-02** (Manager holds **no** accounting write — request the five OD-9 writes explicitly; `procurement:advisory` needs splitting first). ⚠️ **C-23**: the M33 GL collection cannot run, so B5.3's journals surface has no Postman verification. Still no financial statements. **NOT STARTED — needs explicit owner authorisation** |
...
| C | **C-01…C-23** | Backend + discipline gaps, each naming the phase it unblocks | S–L | Each needs explicit per-change authorization. **C-01 + C-02 DONE 2026-08-20** (backend gap batch 1); **C-21 + FU-1 + B3-F1 DONE 2026-08-20** (permissions cutover — `ai/PERMISSIONS_CUTOVER_COMPLETION_REPORT.md`); **PC-03 + PC-04 DONE 2026-08-21** (backend gap batch 2 — `ai/BACKEND_GAP_BATCH2_COMPLETION_REPORT.md`); **C-22** — 37 further guard permissions (franchise/ops/dev/billing/merchant/onboarding/support) still have no seeded row, so those deferred surfaces are 403 for every role, and **B7 must budget the same cutover**; **C-23** — the M33 GL Postman collection cannot run (unresolved `{{accountId}}`), proven pre-existing |
