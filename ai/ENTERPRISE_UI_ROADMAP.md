...
> B5.4-D1 / B5.5-F1).** The owner has decided Manager has full access to everything it is
> responsible for. This reverses the B5.1–B5.5 "Manager is READ-ONLY BY PERMISSION" / PC-01 posture
> described throughout this document: PERMS-2 granted Manager the full 36-string C-21
> accounting/finance set plus the full M28/M29 set (matching Accountant/Owner), also resolving
> **C-27**. **C-25**, **C-26** and **B5.4-D1** are FIXED at the backend. **B5.5-F1** was investigated
> further and confirmed a genuinely dead enum pair (case a — no async worker path exists anywhere in
> `apps/api` that could produce `FAILED`/`PENDING`), NOT implemented. A new phase **B5.7** is added
> below (after B5.6) to retrofit write affordances across every B5.1–B5.5 surface and invert their
> read-only assertions — B5.7 is **NOT started**. B5.6 should be built **write-enabled from the
> start** and therefore needs no B5.7 retrofit. See
> `ai/BACKEND_GAP_BATCH4_COMPLETION_REPORT.md` and CLAUDE.md's C-27 correction note. History below
...
| **B5.4 — Accounting core + Review** | Journal entries (**read-only for Manager — B0 confirmed `journals:create`/`reverse`/`posting:replay` are 403**), posting runs, posting errors, the audit-trail rail. ✅ **B5-F4 FIXED (batch 3, 2026-08-21)** — `/api/audit/timeline` now scopes to `X-Branch-Id` by default (still pages with `pageSize`, unchanged); **C-23** means the journals surface has no Postman verification. *(Was B5.3.)* | M | ✅ **COMPLETE (2026-08-21)** — `ai/ENTERPRISE_B5_4_ACCOUNTING_CORE_COMPLETION_REPORT.md`. Journal entries (list+detail, separate Debit/Credit columns, an independently-computed balance tie-out), Posting runs (list-only, no filter), Posting errors (list+detail) and Audit trail (list-only, scoped to `JournalEntry`/`PostingRun`/`PostingError`) all shipped READ-ONLY (PC-01/OD-9). ⚠️ An operator brief for this phase described fiscal periods/posting-source-maps/tax-config as B5.4 work — the menu tree's own tags (unchanged since B5.1) say B5.5/B5.6, and the phase followed the tags; **neither was touched**. Two new findings, both recorded not fixed: **C-25** (`getJournal` has no branch predicate — mitigated client-side) and **C-26** (`ledger.service.ts`'s `audit.log` calls never stamp `metadata.branchId`, so the Audit trail rail can structurally never surface a ledger-domain event — disclosed in the screen's own copy). **B5.4-D1**: no posting-error resolve/dismiss endpoint exists for any role — disclosed, not treated as a Manager-only gap. Fixtures created live via the API (Owner token) on Tapas Downtown: 2 manual journals (one reversed) + a SUCCEEDED/FAILED posting-run pair. |
| **B5.5 — Closing** | Fiscal periods open/close/lock + period-close runs. **Nimbus's honest analogue of Odoo's Tax Returns + Lock Dates** (NG-17). **PC-07**: four states, `LOCKED` terminal, no unlock; both models are **organisation-level**. Manager can neither close nor lock. *(Was B5.4.)* | S | ✅ **COMPLETE (2026-08-21)** — `ai/ENTERPRISE_B5_5_CLOSING_COMPLETION_REPORT.md`. Fiscal periods and Period close runs (both list-only — neither entity has a `GET .../:id` route) shipped READ-ONLY. All four `FiscalPeriodStatus` stages live-proven in one org (DRAFT/OPEN/CLOSED/LOCKED) via live `open`/`close`/`lock` calls on the isolated stack; `PATCH .../unlock` confirmed 404 (PC-07 terminal). 🔴 **New finding: C-27** — Manager's token additionally holds `pos:accounting:periods:create` (and `accounts:create`, `cost-centers:create`), a pre-existing M28-era seed grant the 2026-08-20 permissions cutover never audited or revoked; live-proven Manager `POST /accounting/periods` → 201. Not fixed (permission/seed change out of scope for a frontend-only phase) — the screen ships zero create control regardless and discloses the gap by name. **B5.5-F1**: `PeriodCloseRunStatus.FAILED`/`.PENDING` are unreachable through the live API — `closeFiscalPeriod()` always creates the run COMPLETED or refuses before any row exists. |
| **B5.6 — Reporting + Configuration** | ~~Aged Receivable/Payable views~~ **shipped early, in B5.2 (2026-08-21)** — pulled forward because they cite the same `ar.aging`/`ap.aging` routes the B5.1 dashboard already reads and the B5.2 brief explicitly scoped them. Remaining: budgets vs actuals, demand calendar; then Chart of Accounts, cost centres, posting source maps, tax config. ⚠️ Procurement suggestions are now **PERMITTED for Manager** (PERMS-2 resolved PC-02 by granting `procurement:advisory:read` in full) and budgets/demand-calendar are **empty on the demo dataset**. ~~⚠️ **B5-F1**: `ar/aging.summary` totals only the returned page.~~ **FIXED (batch 3, 2026-08-21)**. *(Was B5.5.)* ⚠️ **Build this phase WRITE-ENABLED from the start** (PERMS-2, 2026-08-21) — Manager now holds full accounting read+write, so B5.6 needs no B5.7 retrofit. | S | 🔴 NOT STARTED |
| **B5.7 — Accounting write pass** | **NEW (backend gap batch 4, PERMS-2, 2026-08-21).** Retrofits write affordances across every surface B5.1–B5.5 shipped deliberately read-only, now that Manager holds full accounting read+write at the backend (PERMS-2): AP bill create/approve/payment, AR invoice/receipt create, AP/AR credit-note create, recurring-profile create/generate, reminder generate/dismiss, bank account/statement/manual-entry create, reconciliation create/match/skip/complete, journal create/reverse, posting replay, posting-error resolve/dismiss (`B5.4-D1`, now live), fiscal period open/close/lock. Also **inverts** the no-write-affordance assertions those phases added (`manager-b5-assertions.ts` §1–15's write-bans become write-presence checks where a route is now exposed). Runs **after** B5.6 Configuration, since B5.6 is built write-enabled from the start and would otherwise need the same retrofit immediately. | L | 🔴 NOT STARTED — do not begin without explicit owner authorisation |
...
| **C-25** | **`getJournal` has no branch predicate** | Found by Track B5.4 (2026-08-21). `ledger.service.ts`'s `getJournal` resolves by `{id, orgId}` alone — no `branchId` at all, unlike `listJournals` (which filters by branch, with the separate BGB3-L3 defect) and unlike every other accounting detail route in the registry. A journal id from one branch's list is still readable by the SAME id under a DIFFERENT branch's `X-Branch-Id` header — proven live. Mitigated client-side: the journal detail screen compares the returned `branchId` against the active branch and refuses to render a mismatch (`isJournalReadableInBranch`), the same MP0-12 fail-safe B4 used for cross-branch report runs. | Backend | — | S | Recorded, **not implemented** |
| **C-26** | **Ledger audit-log calls never stamp `metadata.branchId`** | Found by Track B5.4 (2026-08-21). `ledger.service.ts`'s six `audit.log(...)` calls (`JOURNAL_CREATED`, `JOURNAL_REVERSED`, `POSTING_RUN_STARTED`, `POSTING_RUN_FINISHED`, `POSTING_ERROR_CREATED`) stamp `metadata.orgId` but never `metadata.branchId`. Backend gap batch 3's B5-F4 fix made `GET /api/audit/timeline`'s default read unconditionally AND `metadata.branchId = X-Branch-Id`, so this rail can **structurally never** surface a ledger-domain event, proven by creating 8 fresh journal/posting-run/posting-error events live and finding every resulting `AuditLog` row's `branchId` NULL (direct `psql` query, not stale seed data). The Audit trail screen discloses the gap by name in its own copy rather than reading as "nothing happened". | Backend | Review → Audit trail (any future consumer of ledger-domain audit events) | S | Recorded, **not implemented** |
| **B5.4-D1** | **No posting-error resolve/dismiss endpoint exists for any role** | Found by Track B5.4 (2026-08-21). `PostingErrorStatus` is a real three-state enum (`OPEN → RESOLVED | DISMISSED`) but grepping every controller in `apps/api/src/modules` finds no mutation on `PostingError` at all — only `pos:accounting:posting-errors:read` is seeded, no `:resolve`/`:dismiss`/`:write` string exists. `OPEN` is therefore effectively terminal for Owner and Accountant too, not only Manager. The Posting errors detail screen states this plainly rather than reusing the "an Owner or Accountant performs this" read-only copy, which would be false here. | Backend | — | S | Recorded, **not implemented** |
| **C-27** | **Manager's token holds a stray, pre-existing accounting WRITE the 2026-08-20 cutover never revoked** | Found by Track B5.5 (2026-08-21). `packages/db/prisma/seed.ts` (~line 1105) seeds Manager with `pos:accounting:periods:create` (and `accounts:create`, `cost-centers:create`) under a M28-era comment reading "Manager: read + create" — this PREDATES the 2026-08-20 permissions cutover (`ai/PERMISSIONS_CUTOVER_COMPLETION_REPORT.md`), which only ADDED 15 new AP/AR/bank-rec/finance read strings and never audited the older M28/M29-era grants for a stray write. Live-proven: Manager `POST /accounting/periods` → **201** on the isolated B5.5 QA stack, creating a real DRAFT period. `PATCH .../open`, `.../close` and `.../lock` remain genuinely 403 — only `:create` leaked. Every prior B5 phase's "5/5 representative writes → 403" spot-check never happened to probe `periods:create`/`accounts:create`/`cost-centers:create`, so this went undetected through B5.1–B5.4. The Fiscal periods screen discloses this by name (`data-accounting-finding="C-27"`) and ships zero create control regardless — the owner's read-first posture for Manager accounting is honored at the UI layer even though the permission grant itself is out of this phase's authority to change. | Backend (seed) | Fiscal periods (disclosed, not exploited) | S | ✅ **RESOLVED (backend gap batch 4, PERMS-2, 2026-08-21)** — no longer a stray leak: the owner decided Manager has full accounting access, so this grant (plus `open`/`close`/`lock`, now also granted) is deliberate and commented as such in the seed. The Fiscal periods screen's `data-accounting-finding="C-27"` disclosure is now a historical footnote, not a live gap — B5.7 will add the create control. |
| **B5.5-F1** | **`PeriodCloseRunStatus.FAILED`/`.PENDING` are unreachable through the live API** | Found by Track B5.5 (2026-08-21). `BankRecService.closeFiscalPeriod()`'s own transaction always creates the resulting `PeriodCloseRun` with `status: 'COMPLETED'` — no branch of that method ever persists `FAILED`, and no code anywhere writes `PENDING` (the Prisma `@default(PENDING)` is a schema default this create call always overrides). A close attempt that cannot proceed throws a `ConflictException`/`NotFoundException` BEFORE any `PeriodCloseRun` row is created at all, proven live: closing an already-CLOSED period returned 409 with zero new rows. The Period close runs screen discloses this rather than implying every enum member is equally attainable. | Backend | Period close runs | S | **Investigated further, backend gap batch 4 (2026-08-21)**: confirmed genuinely dead (case a) — a repo-wide grep found zero `@Cron` decorators, zero BullMQ/queue processors touching `PeriodCloseRun`, and zero other `.update()`/`.create()` calls against the model anywhere in `apps/api`. `closeFiscalPeriod` is the ONLY write path and is fully synchronous. Recorded, **not implemented** — a future batch may propose removing the two dead enum members rather than making them reachable. |
