...
| **B5.2 — Customers + Vendors lists** | AR invoices + AP bills as C4 list views with C5 detail forms; supplier/customer records; the highest-leverage starting point per NG-03. *(Was B5.1.)* | M | 🔴 **NOT STARTED** — needs explicit owner authorisation |
| **B5.3 — Bank reconciliation workbench** | Statements + import + the match/skip/complete reconciliation flow. ⚠️ The demo dataset carries **zero** bank accounts, statements and reconciliations — B5.3 needs a fixture or a generator before it can be designed. *(Was B5.2.)* | M | 🔴 NOT STARTED |
| **B5.4 — Accounting core + Review** | Journal entries (**read-only for Manager — B0 confirmed `journals:create`/`reverse`/`posting:replay` are 403**), posting runs, posting errors, the audit-trail rail. ⚠️ `/api/audit/timeline` ignores `X-Branch-Id` and pages with `pageSize` (**B5-F4**); **C-23** means the journals surface has no Postman verification. *(Was B5.3.)* | M | 🔴 NOT STARTED |
...
| **B5.6 — Reporting + Configuration** | Aged Receivable/Payable views (the **first true financial reports Nimbus can ship**), budgets vs actuals, demand calendar; then Chart of Accounts, cost centres, posting source maps, tax config. ⚠️ Procurement suggestions are **403 for Manager** (PC-02) and budgets/demand-calendar are **empty on the demo dataset**. ⚠️ **B5-F1**: `ar/aging.summary` totals only the returned page. *(Was B5.5.)* | M | 🔴 NOT STARTED |
