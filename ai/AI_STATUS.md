...
- **Prior milestone record (superseded above) — ENTERPRISE UI TRACK B5.3 COMPLETE — Manager
  ...
  Reconciliation — are now real surfaces. **The Accounting menu goes from 12 live rows to
  15** (of 28 total). Manager accounting stays **read-only by permission** — same 15 read
  strings, zero writes (PC-01, re-verified live: 5/5 representative bank writes → 403).
  Reconciliation is Odoo's most action-heavy accounting surface (Match/Skip/Reconcile/
  Validate) and NONE of those controls exist here, not even disabled —
  `AccountingReadOnlyCard` names the denied actions instead.
  **Bank accounts** is list-only (no `bank.account` detail key exists). **Bank statements**
  is list+detail (statement header + full line-level table). **Reconciliation** is
  list+detail (a three-stage `OPEN → IN_PROGRESS → COMPLETED` pipeline, `DISPUTED` as an exit
  chip, statement balance/matched total/**difference**, per-line match evidence). All three
  routes are **PC-06 bare arrays** — the status filter on Bank statements/Reconciliation runs
  entirely CLIENT-side and never reaches the server; no pager binds to any of the three lists.
  The B5.1 Bank dashboard card is now wired for real — all three of its KPIs gained a real
  `drillIn`, replacing their `noDrillInReason` placeholders.
  **Fixtures created live via the API (Owner token) on the isolated stack**, since the demo
  dataset carries zero bank rows by default: 2 bank accounts, 2 statements (5+1 lines), 2
  manual bank entries, 2 reconciliations — one **IN_PROGRESS** with a live-proven **UGX
  6,350,000** non-zero difference (`complete` correctly 400'd) and one **COMPLETED** with a
  zero difference (`complete` returned 200). Rooftop Bar re-verified to carry zero bank rows
  throughout — the empty-branch state exercised in QA is a real read outcome.
  🔴 **One stale B5.1 type field found and fixed**: `BankAccountRow.currentBalance` does not
  exist on the `BankAccount` Prisma model at all — B5.1 never caught this because the card
  only ever rendered a count. Removed; `manager-b5-assertions.ts` §13 now pins its absence.
  **Validated on an isolated local Docker stack** — Postgres `:55450` (`nimbus_b53_qa`), API
  `:4061`, web `:3150`; **shared Neon was never connected to or written** (the isolated API
  held exactly one established TCP connection, to its own local Postgres); both `.env` files
  were **never edited on disk** — the isolated DB target came from an explicitly exported
  `DATABASE_URL`, so SHA-256 is identical before/after by construction. Web typecheck + lint
  (no `--fix`) + production build all pass (3 new pages); **17/17** assertion scripts
  (`manager-b5-assertions.ts` extended with a new §13); `e2e/manager-accounting/bank.spec.ts`
  (new, 16 specs) **64/64 across 4 viewports**; `menu-and-read-only.spec.ts` (updated for
  12→15 rows) **29 passed / 3 skipped** across 4 viewports (the skips are the pre-existing
  "desktop dropdown only at `xl`" reason, not a B5.3 gap); full `e2e/manager-accounting/`
  regression **66/66** at `vp-1440x900`; `e2e/manager-shell/` regression **34/34** (includes
  the cross-role boundary suite); live manual QA toured all 6 new/changed pages across both
  Tapas Downtown (populated) and Rooftop Bar (genuinely empty); zero console errors;
  reconciliation-detail load measured at 5 real GETs + 5 OPTIONS preflights, all GET/OPTIONS,
  zero writes; `/api/health` → ok throughout; `git diff --check` clean. See
  `ai/ENTERPRISE_B5_3_BANK_RECONCILIATION_COMPLETION_REPORT.md`. **B5.4 (Accounting core +
  Review), B5.5 (Closing) and the remainder of B5.6 are NOT started — do not begin any of
  them without explicit owner authorisation.** ⚠️ Note for B5.4: **C-23** — the M33 GL
  Postman collection cannot run (a pre-existing defect), so the journals surface will ship
  without Postman verification.
...
**Prior milestone record (superseded above) — BACKEND GAP BATCH 3 COMPLETE — accounting read-integrity fixes B5-F1…F4 (2026-08-21) — A: COMPLETE / B5.2 UNBLOCKED ON READ INTEGRITY / NOT STARTED / SHARED-NEON DEPLOY STILL GATED.** Track B5.1
  ...
  **Validated on an isolated local Docker stack** — primary Postgres `:55436`, API `:4041`, web
  ...
  regression **21/21** — B2 untouched; `/api/health` → `ok` throughout; all five pre-existing dev
  servers (`:3000/:3001/:3003/:3008/:3009`) verified healthy before and after; `git diff --check`
  clean.
  ⚠️ **Self-inflicted incident, caught and fully reverted before any test relied on it:** the first
  `pnpm lint` invocation used the package's `--fix` script, which reformatted 193 unrelated,
  untouched files across the API tree (pre-existing repo-wide prettier drift, not introduced by this
  batch and not authorised to fix). Caught via `git status` immediately, every collateral file
  reverted to `HEAD` with `git checkout HEAD --`, verified by diffing the modified-file list until it
  matched this batch's own 23-file change list exactly. All subsequent lint checks used targeted
  `eslint` with no `--fix`.
...
- **ENTERPRISE UI RESEARCH COMPLETE + NEW CANONICAL ROADMAP ADOPTED (2026-08-20) — A. COMPLETE.
  ...
  commit/push.**
  **Research.** The owner's live Odoo instance (MARU CREDIT LIMITED, Uganda, Odoo 18-era, dark
  theme) was explored **read-only** through his authenticated browser session — menus opened, list
  and form views opened; **no record created, edited, saved, confirmed or deleted**. Written up as
  `ai/ODOO_REFERENCE_RESEARCH.md` with **17 screenshots** in `ai/odoo-reference-screenshots/`:
  the per-module top bar and click-to-open grouped dropdowns, the **full 58-item Accounting menu
  tree**, the **control-panel row** (`New` + title + cog + chip search + pager + view switcher), the
  three-column Filters/Group-By/Favorites dropdown, breadcrumb + record pager, the dashboard **KPI
  card** anatomy (counts are drill-in links, amounts are data, mixed-weight buttons, and a card may
  have a mini chart *or* a checklist *or* nothing), the Settings two-pane layout, the Employees
  kanban, the user-form credential model, and a **15-component inventory C1…C15**. Surfaces not
  opened are marked **(not opened)**; §6 records what the instance does **not** have (no Point of
  Sale app, no Sales app, the reports themselves not opened).
  **Gap analysis.** `ai/NIMBUS_VS_ODOO_GAP_ANALYSIS.md` — 20 typed gaps **NG-01…NG-20**
  (UI-over-API / Backend / Mixed) with sizing and a top-10 priority list. **Headline finding:
  Nimbus's accounting backend is far larger than any Nimbus document admits.** A route scan found
  four registered, wired controllers — `accounts-payable` (20 routes), `accounts-receivable` (11),
  `bank-rec` (16), `budget` (12) — that `docs/MODULES.md` still marks **"⬜ Planned"**. Roughly **90
  accounting/finance endpoints exist with zero UI**, so most of what the owner admires in Odoo
  Accounting is a **UI gap over an existing API**, not a backend build. ⚠️ Those four modules were
  found by **static scan only** and are explicitly *claimed-by-code, unverified-at-runtime* — an
  M-P0-style live pass has never covered them.
  **Roadmap adopted.** New canonical **`ai/ENTERPRISE_UI_ROADMAP.md`**, three tracks, phased and
  gated, each phase carrying scope / out-of-scope / dependencies / acceptance gates / S-M-L sizing /
  its required completion report. **Track A** experience polish — A0 shipped today (density,
  fullscreen login, terminal identity, short table labels); **A1 is down to one owned item**, the
  shared floor-toolbar wrap at 1024×768, after verifying against the code that status-token
  contrast, the role accents, the jargon-leak string and the Inter webfont bundle are **already
  fixed**. **Track B** the management suite: **B0** API verification extension (docs only, gates
  B5) → **B1** Manager **top-nav shell conversion** (L) → **B2** Overview KPI card grid (M) →
  **B3** Operations + Staff (L) → **B4** Reporting (M, CSV-only) → **B5** Accounting suite (L,
  sub-phased B5.1–B5.6 over a 7-menu tree adapted from Odoo) → **B6** Settings (M) → **B7** Owner
  variant (M). **Track C** the true backend/discipline gaps **C-01…C-20**, each naming the Track B
  phase it unblocks; **Track C-P** carries Cashier **C4→C6** forward unchanged and in parallel.
  Hard blocks recorded: **C-02** (the `/hr/employees` compensation leak) blocks B3's staff
  directory; a **B0 go** blocks B5; **C-03** (no report rows) blocks B4's graph/pivot. Soft blocks
  degrade honestly: **C-04** (SSE) → B2 polls; **C-01** (fake PDF) → B4 ships CSV only; **C-15**
  (no saved views) → Favorites is localStorage-only or absent.
  ⚠️ **OWNER DECISION RECORDED — `docs/DECISIONS.md` D-MGRTOPNAV: management navigation switches to
  an Odoo-style TOP NAV BAR, superseding the M-P1 bottom-nav decision for Manager.** Module bar +
  click-to-open dropdown submenus, a control-panel row and breadcrumb + record pager. **Waiter,
  Cashier and Supervisor keep bottom nav and must render byte-identically.** **Only the navigation
  presentation is superseded** — M-P1's shell, `ManagerSessionGuard`, the **branch switcher** (source,
  persistence, `X-Branch-Id` plumbing, narrow `["manager", …]` invalidation), the **surface
  allow-list**, the honest foundation pages, the real Manager **Me** and the "three verified chips
  only" readiness rule all carry forward, and M-P0's **MP0-01…MP0-18** remain in force. The top nav
  ships as an **additive variant of the shared `OperationalShell`**, never a Manager fork.
  **Docs updated:** `ai/ENTERPRISE_UI_ROADMAP.md` (new); `docs/DECISIONS.md` (**D-MGRTOPNAV** +
  **D-ENTERPRISE**, both owner-approved and dated, with D-NAV's Manager row struck through rather
  than deleted); `ai/MANAGER_RECONSTRUCTION_ROADMAP.md` (supersession banner + M-P2 and
  dependency-graph annotations; **M-P0/M-P1 history intact**); `docs/UI_SYSTEM.md` §3 + new §3b;
  `docs/DOCUMENT_INDEX.md` (new Enterprise UI push section + the Manager-roadmap row reclassified);
  `PROGRESS.md`; this file; `CLAUDE.md` + `CODEX.md`.
  **NEXT = Track B1 (Manager top-nav shell conversion) — pending an explicit owner go. Nothing in
  Track B is implemented.** Eleven open owner decisions (**OD-1…OD-11**) are recorded with
  recommendations in the roadmap §7; **OD-4** (sub-desktop collapsed presentation) and **OD-5**
  (additive shell variant vs a separate management shell) must be answered at the start of B1.

- **OWNER UI POLISH WAVE 2 — GLOBAL DENSITY + FULLSCREEN LOCK SCREEN COMPLETE (2026-08-20) — A.
  COMPLETE. Frontend + docs only; no backend/API/schema/migration/seed/permission/Postman change;
  no commit/push.** Five owner complaints fixed entirely in the SHARED layer (no role fork):
  **(1) `/login` no longer scrolls** — it is a true `h-screen` + `overflow-hidden` layout whose card
  scrolls internally if ever needed. Live proof (`document.documentElement.scrollHeight` vs
  `window.innerHeight`): **680/680, 768/768, 900/900, 1080/1080** at 1280×680, 1366×768, 1440×900,
  1920×1080.
  **(2) Global density mechanism** — `html { font-size: clamp(13.5px, calc(0.625vh + 9.25px), 16px) }`
  in `globals.css` plus rem-normalized `--space-*` tokens, so every rem-based Tailwind utility scales
  with viewport height; measured root font size 13.50 / 14.05 / 14.88 / 16.00px across the matrix
  (16px ceiling = the unchanged pre-density baseline). Targeted px fixes where rem cannot reach:
  icon registry **18/24/32 → 16/20/28**, table card **`min-h-[176px]` → `min-h-[9.5rem]`**
  (measured 152 / 141 / 128px), grid track `minmax(220px,1fr)` → `minmax(13rem,1fr)`. Shared regions:
  header **80→64px**, brand + identity tiles 44→36px, readiness 44→36px, main top padding 160→116px,
  bottom nav **80→64px**, bottom clearance 112→80px, `PageShell` title `text-2xl`→`text-xl`.
  Uniform across all four roles; `pos-shell/layout.ts` documents the rem class behind each number.
  **(3) Brand-led lock screen** — logomark tile + inline Inter-ExtraBold "Nimbus POS" lockup (≈41px
  tall at 1440), "Service terminal" heading, three compact Workstation/Time/API chips. The
  role-marketing paragraph and the always-visible footer sentence were **removed**; the truthful
  blocked-role `StatusMessage` behaviour is unchanged.
  **(4) Terminal identity** — new shared `components/pos-shell/station.ts` replaces the dead
  "Service area unavailable" / "Workstation unavailable" fallbacks with a deterministic station
  label (`localStorage["nimbus.stationTerminalLabel"]`, else **"Terminal 01"**), hydration-safe and
  documented in code as a station label, **not** backend data. Verified live in all four role
  headers, the Cashier Till toolbar and the Cashier/Supervisor Me screens.
  **(5) Short floor table labels** — deterministic display-side abbreviation in the shared
  `floor/formatters.ts` (`QA-OPEN-01→QO-01`, `QA-P4-CLEAN-02→QP4C-02`, `QA-P4-PASS2-1440→QP4P2-1440`,
  `QA-PRE-BILL-01→QPB-01`; ≤7 chars untouched), collision-safe within a fetched set via
  `buildOperationalTableLabelMap()`, with the FULL label preserved in `title`/`aria-label`. Card
  titles are now one line. Applied to floor cards (all roles), selected-table workspace headers,
  cashier bill panels, reservation/table selects and find-bill/split/transfer dialogs.
  **Two assertions updated to the new canonical values** (both noted in `docs/DECISIONS.md`):
  `shell-assertions.ts` bottom-nav icon size 24→**20**; `floor-assertions.ts` card title
  `break-words`→**`truncate`** (full-identifier guarantee re-asserted through `title` + `aria-label`).
  **Validated 2026-08-20:** web typecheck + lint pass (`next build` deliberately not run);
  **12/12** assertion scripts pass; Playwright **180/180** — `e2e/manager-shell` **92/92**,
  `e2e/cashier-floor` navigation + shared-floor-parity + responsive **32/32**, `e2e/supervisor-prompt3`
  floor + regression + responsive **32/32**, supervisor approvals/reservations navigation + responsive
  **24/24** — all across the four viewport projects; **zero console errors** across every captured
  surface; 15 screenshots at 1280×680 + 1440×900 in `/tmp/qa-shots/polish2/`. **Known gap:** the live
  demo branch seeds only short labels (TD-01…TD-22), so the abbreviation was proven live by rewriting
  the `GET /api/tables` RESPONSE in the browser (a display-side test double — no demo-data change)
  plus executable assertions. Docs: `docs/UI_SYSTEM.md` §1c/§1d/§2b/§4/§5/§5b, `docs/DECISIONS.md`
  D-DENSITY / D-LOGIN / D-TERMINAL / D-TABLELABEL.

- **MANAGER RECONSTRUCTION — PROMPT M-P1 COMPLETE (2026-08-20) — A. M-P1 COMPLETE / READY FOR
  M-P2. Frontend + docs only; no backend/schema/migration/seed/permission/Postman change; no
  commit/push.** Manager joined the shared operational UI system as its **fourth consumer** (never
  a fork): `"manager"` added to `OperationalRole` + `role-navigation.ts`; the locked six-tab nav
  **Overview · Operations · Staff · Reports · Settings · Me** (no More tab, no Approvals tab) in
  `lib/manager/routes.ts` with landing `/manager/overview` and a `/manager` redirect; six new
  canonical icon-registry names (`overview` ChartLineUp, `operations` ListChecks, `staff`
  UsersThree, `reports` FileText, `settings` GearSix, `caretDown` for the switcher);
  `components/manager/shell/*` as thin adapters over `OperationalShell` / `OperationalHeader` /
  `OperationalBottomNav` / `OperationalIdleLogoutHandler`; `ManagerSessionGuard` sending non-manager
  users to `/login?reason=manager_only`; and the **branch switcher** — the one genuinely new shell
  affordance — mounted through a new **optional** `OperationalHeaderContext.branchSwitcher` slot so
  Waiter/Cashier/Supervisor headers render byte-identically. Branch state
  (`lib/manager/branch-model.ts` + `branch-context.ts`) resolves stored → `defaultBranchId` →
  default-flagged → first ACTIVE membership, persists at **`nimbus.managerBranchId`** (deliberately
  separate from the station key `nimbus.stationBranchId`), flows into `X-Branch-Id` via the existing
  `apiRequest({ branchId })` parameter (**no API-client change**), and invalidates **only** the
  `["manager", …]` namespace — never `queryClient.clear()`, never auth/profile. `lib/manager/
  permissions.ts` is a **surface allow-list, NOT a permission check** (M-P0 §12: the manager JWT
  holds 214 permissions including `pos:hr:compensation:read`, `pos:hr:contracts:*`,
  `approvals:decide`, `tenancy:membership:manage` — a `hasPermission()` UI would open payroll-adjacent
  surfaces). Pages: six foundation screens stating scope + phase + verified backend limits with **no
  fabricated data**, plus a real Manager **Me** built solely from the already-fetched
  `/api/auth/me` (no HR read — that endpoint returns compensation on the wire). Readiness ships
  **three verified chips only** (Branch, `GET /reports/catalog` → 24 of 37 generators, `GET /devices`
  → branch-scoped total); **tills/shifts/pending-approvals chips are omitted, not faked.** Fourth
  navy-family role accent added (`--color-role-manager` `oklch(0.36 0.06 324)` → `#4D324F`,
  white-on-solid **11.18:1**; soft `oklch(0.965 0.015 324)`), mapped in Tailwind + `roleAccentMap`.
  Validated 2026-08-20: web typecheck + lint pass (`next build` deliberately NOT run in the dev QA
  sandbox and stated as such); `manager-p1-assertions.ts` plus **11/11** existing assertion scripts
  pass (`shell-assertions` and `profile-assertions` extended to four roles); Playwright
  `e2e/manager-shell/` **92/92** across `vp-1024x768`/`vp-1366x768`/`vp-1440x900`/`vp-1920x1080`;
  cross-role regression (`e2e/supervisor-prompt3/{regression,role-boundaries}` +
  `e2e/cashier-floor/{role-boundaries,navigation-and-default-route,cross-role-c2-regression,
  till-and-me-regression}`) **68/68**; live manager browse at 1440×900 + 1024×768 (all six tabs, the
  switcher open, a branch switch with the `X-Branch-Id` change captured, persistence across reload
  and route change, zero console errors, zero horizontal overflow, one `/auth/me` per page load);
  Waiter/Cashier/Supervisor re-verified live — unchanged landings, navs and headers, no switcher.
  `GET /api/health` `ok`; `git diff --check` clean of new issues. Three findings recorded and **none
  implemented**: the shared role-guard "Return to login" can lose its `reason` query to the guard's
  own session effect (pre-existing in all four guards), the header branch label truncates at
  1024×768, and the switcher drops a `(default)` suffix in favour of a badge on Me. **M-P2 (Overview
  dashboard) NOT started — do not start without explicit authorization; when authorized it inherits
  count-only tills/shifts, client-filtered approval counts, a `fetch`+ReadableStream SSE reader, and
  qualified gross/net labels.** See `ai/MANAGER_P1_SHELL_COMPLETION_REPORT.md`.

- **CASHIER FLOOR-FIRST RECONSTRUCTION — PROMPT C3 COMPLETE (2026-08-20) — A. C3 COMPLETE /
  READY FOR C4. Frontend-only; no backend/schema/migration/seed/permission/Postman change; no
  commit/push.** C3 turns the C2 read-only settlement workspace into a working, **fail-closed
  payment + close** surface by **mounting** the already-verified checkout/split primitives rather
  than rewriting them. New: `apps/web/src/components/cashier/floor/CashierSettlementActions.tsx`
  (composes `checkout/CashierPaymentPanel` — payment entry, amount + readiness validation, blocked
  banner, result notice, payment history, `CashierCloseOrderPanel` — and
  `resolution/CashierResolutionPanel` with the **additive** `variant="split-only"` prop →
  `CashierSplitBillPanel` + `CashierSplitItemsPanel`; `CashierAdvancedResolutionPanel`
  (merge/move-items/transfer-table) is deliberately **not** mounted) and
  `apps/web/src/lib/cashier/settlement-mutations.ts` (the only post-mutation refresh: **awaits** a
  canonical refetch of `orderDetail` + `orderPayments` before any result is presented — no
  optimistic money — then narrowly invalidates `tableBills`, `floor`, the `find-bills` prefix and
  the `["waiter"|"supervisor","floor",branchId]` keys; **no** bare `invalidateQueries()`, no
  `["cashier"]` root sweep, no menu/profile/auth/receipts/refunds/queue/reservations touch).
  Delivered capabilities: **cash payment settles AND closes** in one call at the single verified
  choke point `POST /api/pos/orders/:id/close`; **card / MTN / Airtel / bank manual-reference**
  payments via `POST /api/payments/manual-reference` (a final one auto-settles server-side);
  **partial payment** with a canonical remaining balance re-read from the backend; **split-bill**
  allocation (metadata only, truthful "no child orders" copy) and **split-items** child order
  (`NEW`, correctly not offered as payable); truthful terminal state — a CLOSED/VOIDED bill renders
  **no settlement control at all**. Everything fails closed: shift not `active` (covers
  loading/unavailable/failed/inactive), till not `active` for cash (a till owned by another operator
  is simply absent from `GET /tills/active` → blocked), unresolved payment summary, pending provider
  intent, terminal order, amount > outstanding, or a non-`SERVED` bill for cash close.
  ⚠️ **Documented deviation — no standalone Close button.** `CloseOrderDto.payments` is
  `@ArrayMinSize(1)` and `closeOrderWithPayment` requires `SERVED` + full coverage, so a
  zero-payment close is impossible and "settled but awaiting close" is unreachable; close is
  therefore reached *through* payment and `CashierCloseOrderPanel` states the real precondition.
  Exposing a synthetic close would need a backend change C3 was not authorized to make.
  **Validated:** web typecheck PASS, lint PASS (0/0); `next build` **not run** (forbidden in the QA
  sandbox — stated, not implied); shell/floor/profile/cashier-c1/cashier-c2/**cashier-c3**
  assertions PASS; Playwright `e2e/cashier-floor/` **192 passed / 0 failed / 0 skipped** (48 tests ×
  4 viewports, 26 spec files) and cross-role regression **20/20** (Waiter/Supervisor unaffected),
  executed against an **isolated disposable local Postgres** stack with REAL payments/closes
  (`GET /api/health` → ok); console/network clean; 36 screenshots at 1440×900 + 1024×768, all
  viewed. **Six findings recorded, none implemented** — F-C3-1 `generateOrderNumber` can 500 on
  branch-prefixed demo numbers (matrix **M8**); F-C3-2 `POST /payments/manual-reference` accepts a
  payment on an already-CLOSED order (matrix **M7**, UI fails closed); F-C3-3 cashier idempotency
  keys are not reused across retries (the shared `pos-shell` intent model is); F-C3-4 reservation
  auto-completion does **not** fire on the cashier close path (it lives in
  `OrdersService.transitionOrder`, which `closeOrderWithPayment` bypasses); F-C3-5 split allocations
  can produce sub-unit UGX amounts, rendered truthfully; F-C3-6 the readiness-strip
  "Read-only readiness" badge is now ambiguous. **C4 NOT started** — receipt print/reprint/deliver,
  receipt search, refund execution and Receipts retirement remain gated, as do Queue retirement (C5)
  and closure (C6). See `ai/CASHIER_FLOOR_RECONSTRUCTION_C3_SETTLEMENT_COMPLETION_REPORT.md` +
  `ai/CASHIER_FLOOR_RECONSTRUCTION_C3_QA_EVIDENCE_INDEX.md`; docs updated in
  `docs/cashier-ui-docs/{README,CASHIER_RECONSTRUCTION_ROADMAP,CASHIER_API_MATRIX,CASHIER_LIFECYCLE,
  CASHIER_COMPONENT_REUSE_MAP}.md`.

- **MANAGER OWNER DECISIONS APPROVED — MANAGER TRACK UNBLOCKED (2026-08-20). Documentation only;
  no code, no backend/schema/migration/seed/permission/Postman change; no commit/push.** The
  product owner (Moses) approved the Manager core + MVP scope **as recommended**: role
  `JobRole.MANAGER` (no separate Branch Manager role); landing `/manager/overview`; bottom nav
  exactly **Overview · Operations · Staff · Reports · Settings · Me** (**no More tab, no Approvals
  tab**); a **required** branch switcher in the shell/header driving **every** branch-scoped query;
  approval **counts** on Overview with escalations integrated into Operations and leave/swap review
  into Staff; Reports + Settings first-class. Scope: Staff = directory (safe fields) + frontline
  onboarding + Quick PIN admin + leave review + shift-swap review, **compensation/contracts/payroll
  EXCLUDED**; Operations = **read-only oversight** of tables/orders/tills/shifts/reservations with
  **no cashier-checkout and no waiter-order-entry clone**; Reports = catalog + DTO-verified generate
  + history/detail + export **with a truthful generator-unavailable state, fake downloads
  forbidden**; Settings = branch profile + device registry, printer routes **metadata-only**,
  terminal pairing **stub-only**, alert rules **defer-or-read-only**, sync-conflict diff
  **deferred**, owner/admin + SaaS billing **excluded**. **Domain-specific decision routes are
  preferred** over the generic `POST /api/approvals/:id/decide`, aligning with the **Supervisor
  Option B precedent**.
  ⚠️ **Sequencing changed by the same decision:** the rule *"Manager reconstruction is blocked until
  Cashier reconstruction closes at C6"* is **REPLACED** — **Cashier C3 is authorized to proceed in
  parallel** (tracked separately) and the **Manager track no longer waits for Cashier C6**. Older
  wording in `CLAUDE.md` §10/§12, `CODEX.md`, and the Cashier roadmap's scope lock is **stale on
  sequencing only**.
  **Artifacts:** `Front End/manager_ui_full_docs_pack/manager-ui-docs/MANAGER_APPROVAL_DECISIONS.md`
  — all 42 previously "Pending owner approval" cells now **Approved (owner, 2026-08-20)**, the 8
  §8 Safety rows stay **Locked**, the §9 checklist is fully ticked, and a dated header records the
  approval session + the sequencing replacement. **NEW canonical
  `ai/MANAGER_RECONSTRUCTION_ROADMAP.md`** — seven phased prompts **M-P0 → M-P6** (M-P0 repo/API
  verification audit incl. dash endpoints, SSE `/api/stream/metrics`, and the report generators;
  M-P1 shell/nav/session-guard/branch-switcher with MANAGER wired into the `pos-shell` role
  registry and login routing; M-P2 Overview; M-P3 Operations read-only; M-P4 Staff; M-P5 Reports;
  M-P6 Settings + Me + closure QA), each with scope, out-of-scope, key files, validation gates
  (typecheck/lint/build, assertion scripts, **Playwright 4-viewport matrix**, **isolated-DB rule**)
  and its locked constraints; it also records the **shared-shell reuse rule** (Manager is the **4th
  consumer** via thin adapters — **never a fork**; Floor-like views reuse `OperationalFloor`
  read-only), the rebrand reference (`NimbusLogomark`, brand tokens, navy `#000033`), and **7
  recorded contradictions** between the pack docs and the locked decisions. `docs/manager-ui-docs/`
  reconciled into a **portable canonical set** — every dead `file:///C:/Users/arman/...` link
  replaced with repo-relative paths, status/date header + doc index with an explicit authority
  split vs. the fuller pack, dated decisions-approved + rebrand notes; `MANAGER_API_MATRIX.md`,
  `MANAGER_LIFECYCLE.md`, and `MANAGER_GAP_REGISTER.md` got dated header notes (annotated, **not**
  rewritten). `docs/DOCUMENT_INDEX.md` updated with the decisions register, the roadmap, and the
  reconciliation status.
  **Finding recorded during this pass:** `packages/db/prisma/seed.ts` grants **MANAGER both
  `approvals:read` (line 974) and `approvals:decide` (line 975)** (`Manager:` block at line 758,
  "BG2 … Manager: full"), while the `Supervisor:` block (line 1090) grants **neither**. So unlike
  Supervisor, Manager is *not* permission-blocked from the generic approvals inbox — the
  domain-route preference is a **product/safety constraint the frontend must enforce**, not a
  permission block (risk already registered as MANAGER-GAP-007). **Do not remove those seed grants**
  without explicit authorization.
  **Manager UI implementation remains NOT STARTED.** Approval of decisions is **not** authorization
  to write Manager runtime code — **M-P0 runs first** and every phase keeps its own gate.

- **REBRAND + ROLE UI QA WAVE COMPLETE (2026-08-20).** Frontend + documentation only; **no
  backend/schema/migration/seed/permission/Postman change; no commit/push.** (1) The **Aug-2026
  Nimbus POS Brand Identity** (designer Andimashimwe Rhoda) fully landed: canonical tokens in
  `apps/web/src/styles/globals.css` (navy-950 `#000024` / **navy-900 `#000033`** canonical, RGB
  `0 0 51` / navy-800 `#1E1E52` / silver `#B3B4AF` / graphite `#6B6B6B` — sampled from the guide's
  swatch, the guide's printed hex is a typo); `surface-navy` + `focus-ring` now resolve to navy-900;
  a **NEW alpha-channel token system** (channel triplets + `tailwind.config.ts`
  `rgb(var() / <alpha-value>)` mappings) that **FIXED a pre-existing app-wide defect** — every
  `token/alpha` utility (all modal scrims, e.g. `bg-brand-navy-950/40`) previously rendered fully
  transparent. Steering-wheel logomark extracted as **true vectors** from the brand PDF into
  ...
  mark** — the documented exception to the canonical icon registry) mounted in `BranchContextLabel`
  (44px header tile) and `login.tsx` (56px hero tile, replacing `LockKey`); `_app.tsx` gained
  theme-color `#000033`, manifest, apple-touch-icon, og meta. New canonical `docs/BRAND_IDENTITY.md`;
  live design docs rebranded, historical ones given supersession banners, `docs/DOCUMENT_INDEX.md`
  updated. (2) **Accessibility/consistency fixes** in shared components (verified in all 3 roles at
  1440×900 + 1024×768): `ui/Button.tsx` new **`inverse`** variant + `disabled:text-text-primary`
  (disabled contrast **3.62 → 8.51:1**); `OperationalHeader` logout on the inverse variant
  (**2.71 → 20.48:1**, off-palette hover → navy-800); new `--shadow-focus-inverse` wired into
  `OperationalBottomNav` + Button inverse (the focus ring was navy-on-navy = invisible on
  header/nav); `ActionConfirmDialog` + `CashierFindBillDialog` + `SupervisorFindOrderDialog` +
  `SupervisorCreateReservationDialog` scrims `bg-black/40` → `bg-brand-navy-950/40`;
  `CashierPaymentMethodSelector` selected-tile class conflict fixed (label was **1.18:1** =
  invisible on the legacy queue route); `CashierCheckoutPreview` `text-white` → `text-text-inverse`.
  (3) **WAITER pass complete** — all surfaces QA'd live at 2 viewports (floor, workspace/order
  builder, reservations, me, receipt drawer, login), flows confirmed, **zero console errors**,
  **37 endpoints verified** (20 GETs + 11 writes live, rest static); NEW canonical
  `docs/waiter-ui-docs/{WAITER_API_MATRIX,WAITER_LIFECYCLE,README}.md` (Waiter previously had **no**
  canonical docs dir and **no** API matrix anywhere). (4) **CASHIER pass complete within the C2
  boundary** — zero/one/multiple bill resolution (fail-closed, bounded queries, URL state all
  confirmed), Till, Me, hidden Queue/Receipts compatibility routes confirmed rendering; NEW canonical
  `docs/cashier-ui-docs/CASHIER_API_MATRIX.md` (32 endpoints, **19 live-verified**) + README update;
  legacy pack matrix superseded. **C3 NOT started; nothing gated was implemented.** (5) **SUPERVISOR
  pass complete** — floor + action dialogs opened/cancelled, reservations 4 views + create, approvals
  4 domains incl. reject-only shift-swap, me; reservations **one bounded query** design confirmed;
  legacy `/supervisor/orders` redirect confirmed; `SUPERVISOR_API_MATRIX.md` live-verified (**68
  rows: 24 live + 3 probes + 41 static**) and its **quick-pin path defect FIXED** (real route
  `auth/quick-pin-login`); `docs/UI_SYSTEM.md` §9 idle-logout claims **corrected** (Supervisor DOES
  inject the shared idle handler — code wins); 7 supervisor docs annotated. **Validated (all
  executed, final run 2026-08-20):** web typecheck + lint + production build all **PASS**;
  Playwright-driven visual QA **~180 screenshots** across roles/viewports; `git diff --check` clean
  except pre-existing md whitespace. **QA environment:** isolated local **Docker-free Postgres 16 +
  WASM-Prisma harness** + API `:3001` + web dev `:3000`; **shared Neon untouched**; destructive QA
  never ran against shared Neon. ⚠️ **Harness caveat:** the two "500 defects" first seen (receipts
  `GET`, add-item `POST`) were **QA-harness artifacts** (WASM Prisma Decimal class identity), fixed
  in the harness and **re-verified 200/201 — NOT product bugs.** The only `packages/db` change is the
  generator line `previewFeatures = ["driverAdapters"]` required by that harness (no model/enum/
  migration/seed change). **OPEN findings recorded for the owner, none implemented:** (a) Cashier has
  **no shift-open control** while `tills.service` requires an actor-owned active shift → cold-start
  cashier cannot open a till unaided, and `/shifts/active` vs `/tills/active` key on different fields
  → contradictory readiness chips (CASHIER_API_MATRIX **M1**); (b) status-warning-on-warning-surface
  contrast **2.37:1** (+ success 3.94, danger 3.83) — status tokens deliberately unchanged, owner
  decision pending; (c) six `--color-role-*` oklch accents read off-brand vs the new navy — owner
  decision pending; (d) `--color-status-neutral #616367` now decoupled from graphite `#6B6B6B`;
  (e) zero-bill "closed bills" list reuses `CashierBillSelector` copy ("N payable bills are open") —
  untruthful, fix in C3/C4; (f) Supervisor discount dialog placeholder leaks internal jargon
  ("Prompt 3B3A discount validation"); (g) `OperationalFloor` toolbar wraps awkwardly at 1024×768
  (all roles); (h) no waiter Quick PIN seeded for `waiter@nimbus.demo` though docs advertise 246810;
  (i) Inter webfont still not bundled (system-font fallback). **Cashier C3 remains gated (pending
  authorization) and Manager reconstruction remains NOT STARTED and blocked until Cashier C6 —
  the owner decisions in `Front End/manager_ui_full_docs_pack/manager-ui-docs/
  MANAGER_APPROVAL_DECISIONS.md` are all still pending.** See
  `ai/REBRAND_AND_ROLE_QA_COMPLETION_REPORT.md` (canonical record) and `docs/BRAND_IDENTITY.md`.

- **CASHIER FLOOR-FIRST RECONSTRUCTION — PROMPT C2 COMPLETE (2026-07-31) — A. C2 COMPLETE /
  READY FOR C3.** Frontend-only; **no backend/schema/migration/seed/permission/Postman change; no
  commit/push.** C2 replaces C1's neutral selected-table boundary with **table→bill resolution + a
  canonical read-only settlement-workspace foundation**. New `lib/cashier/bill-resolution.ts`
  (central fail-closed classifier: PAYABLE / PAYMENT_IN_PROGRESS / PARTIALLY_PAID / SETTLED /
  TERMINAL_READ_ONLY / NOT_CASHIER_SETTLEABLE / UNKNOWN_UNSAFE) + `lib/cashier/bill-query-keys.ts`
  (narrow read domains) + `lib/cashier/floor-route.ts` (`buildCashierBillQuery`/`clearCashierBillQuery`;
  C1 `buildCashierFloorQuery` unchanged). Selecting a table runs ONE bounded branch-scoped
  `GET /pos/orders?tableId=&pageSize=50` and resolves **zero** (truthful empty + read-only closed
  list) / **one** (auto-resolve, URL gains `orderId`, no selector) / **multiple**
  (`CashierBillSelector`, never a silent first-pick). Canonical URL `?tableId=&orderId=` (or `?orderId=`
  tableless/takeaway/Find bill); refresh/Back/Forward safe; invalid/cross-branch orderId fails safe.
  ONE `CashierSettlementWorkspace` (read-only: Bill/Totals/Payment state/Settlement readiness/History)
  reuses `CashierOrderTotals`, `CashierPaymentSummary`, `normalizeCashierOrder` — no payment/split/
  close/receipt/refund control; payment state fails closed. `CashierFindBillDialog` = bounded
  Cashier-only sibling above the shared Floor (tableless+takeaway, exact-id fallback; receipt-ref
  deferred to C4). Queue/Receipts remain hidden compatibility routes (not deleted/redirected/mounted).
  Validated: web typecheck/lint/build; C1+C2+shell+floor assertions; Playwright `e2e/cashier-floor/`
  executed on an **isolated local Docker Postgres** stack (API :4001 / web :3100 / Postgres :55432;
  shared Neon untouched). See `ai/CASHIER_FLOOR_RECONSTRUCTION_C2_BILL_RESOLUTION_COMPLETION_REPORT.md`
  + `ai/CASHIER_FLOOR_RECONSTRUCTION_C2_QA_EVIDENCE_INDEX.md`. **C3 (payment/close execution) NOT
  started.**

- **CASHIER FLOOR-FIRST RECONSTRUCTION — PROMPT C1 COMPLETE (2026-07-31) — A. C1 COMPLETE /
  READY FOR C2 (superseded by C2 above).** Frontend-only; **no backend/schema/migration/seed/permission/Postman change; no
  commit/push.** Cashier is now the third shared-`OperationalFloor` consumer (Waiter/Supervisor/
  Cashier). Delivered: visible nav changed to **Floor · Till · Me** (`lib/cashier/routes.ts`,
  Floor uses the shared `floor` icon); new `pages/cashier/floor.tsx` + `pages/cashier/index.tsx`
  redirect (`/cashier` → `/cashier/floor`); `getCashierLandingPath()` → `/cashier/floor`; new
  `components/cashier/floor/CashierFloorScreen.tsx` renders the shared `OperationalFloor` (no
  forked card/grid/toolbar) with a Cashier data layer `lib/cashier/floor-{api,model,route}.ts`
  over already-held `pos:table:read`/`pos:orders:read`/`pos:reservation:read` (one bounded query
  domain: tables + active orders + reservations; no per-table payment fetch); canonical `?tableId=`
  selection URL state (push-then-replace, `shallow:true`; refresh/Back/Forward safe; invalid table
  → fail-safe "Table unavailable"); and a **read-only, truthful** selected-table boundary
  (`CashierSelectedTablePanel`, "Select a bill to continue.") exposing **no** payment/close/split/
  refund/receipt action — the mount point C2 replaces. **Queue and Receipts are NOT deleted and
  NOT redirected** — hidden compatibility routes reachable by direct URL only (retire Receipts→C4,
  Queue→C5). Till/Me unchanged. No shared `pos-shell`/`floor` component was modified; Waiter and
  Supervisor Floors unchanged. **Validated (all executed):** web typecheck + lint + build; static
  assertions `npx tsx apps/web/scripts/{shell,floor,cashier-c1}-assertions.ts`; Playwright
  `apps/web/e2e/cashier-floor/` **88/88** (22 tests × 4 viewports 1024/1366/1440/1920) + cross-role
  regression (Waiter/Supervisor nav + shared Floor) **40/40**, executed on an **isolated local
  Docker Postgres** stack (Postgres :55432, API :4001, web :3100 built with
  `NEXT_PUBLIC_API_BASE_URL=http://localhost:4001`; isolation verified by READ before any test;
  stack torn down); `git diff --check` clean; shared Neon never written. **C2 (table→order
  resolution + settlement workspace foundation + Find bill foundation) NOT started.** Reports:
...
- **CASHIER FLOOR-FIRST RECONSTRUCTION — PROMPT C0 COMPLETE (2026-07-31).** Documentation-and-
  verification-only pass (no runtime/backend/schema/permission/Postman change; no commit/push).
  Safely fetched and fast-forward merged `origin/docs/cashier-three-tab-floor-workflow` (head
  `9b374c39b8cc893a26c0dc374418ca008296a13c`, confirmed by SHA) into the dirty local worktree — 11
  new additive documentation paths (`docs/cashier-ui-docs/*`, `ai/CASHIER_FLOOR_RECONSTRUCTION_
  {DECISION,GAP_REGISTER,PROMPT_C0}.md`), zero overlap with the ~50 locally modified/untracked
  paths already in the worktree (Supervisor Approvals Prompt 5A/5B1/5B2 work), zero conflicts. All
  12 intentional shared-Floor-system deletions (11 per-role `WaiterTable*`/`SupervisorFloor*`/
  `SupervisorTable*` files + shared `CurrentTime.tsx`, superseded by the shared UI system in commit
  `24c7332`) confirmed still absent post-merge. Read the new canonical Cashier docs plus root
  governance docs; dispatched five parallel audits of the actual current-worktree Cashier
  implementation (routes/nav/shell; shared-Floor interface + full component classification;
  Queue/Receipts/payment/table-to-order/Till capability migration matrix; permission/API contract
  matrix; test inventory + cache/performance topology) against the locked target: Cashier nav
  **Floor · Till · Me** (from current Queue/Receipts/Till/Me), default route `/cashier/floor`,
  Cashier as the third `OperationalFloor` consumer, a settlement/payment/close/receipt workspace
  behind table selection, and a compact **Find bill** sibling control for tableless/takeaway/
  direct-lookup/receipt-reference/closed-order cases — reconstruction is 7 prompts C0–C6. `git diff
  --check` clean; `corepack pnpm@8.15.0 --version` → `8.15.0`; no package/API/schema/migration/
  seed/permission/Postman file touched. Reports:
  ...
  CAPABILITY_MIGRATION_MATRIX,PERMISSION_AND_API_MATRIX,TEST_INVENTORY,PROMPT_C1}.md`, updated
  `ai/CASHIER_FLOOR_RECONSTRUCTION_GAP_REGISTER.md`. **C1 NOT started — do not begin without
  explicit authorization.** Manager reconstruction remains blocked until Cashier C6 closes. See
  `PROGRESS.md`, `CLAUDE.md` §9/§10.

- **SUPERVISOR RECONSTRUCTION FINAL CLOSURE — B. COMPLETE WITH KNOWN LIMITATIONS / DEMO-READY
  (2026-07-31).** Integrated final QA across the entire Supervisor experience (Floor/order-
  workspace, Reservations, Approvals, Me), Waiter/Cashier cross-role regression, and role/privacy
  boundaries, on an isolated stack: a disposable Neon branch (`br-curly-wind-a47576qd`,
  fail-closed isolation via the Prompt 4D tooling) for the live API mutation matrices
  (reservation **53/53**, approvals **29/29**), and a local disposable Docker Postgres
  (`nimbus-supfinal-qa`, port 55433 — migrated/seeded/demo-imported) for the four-viewport
  Playwright browser suite after the Neon-branch path proved too slow/unstable for sustained
  browser automation (20–27s/test round-trips + a Windows Chromium `STATUS_STACK_BUFFER_OVERRUN`
  worker-crash pattern — consistent with the already-documented Prompt 4D Neon-latency finding).
  **Browser QA: 262/264 executed passed across vp-1024x768 (64/66, 2 graceful data-exhaustion
  skips later confirmed green) / vp-1366x768 (66/66) / vp-1440x900 (66/66) / vp-1920x1080
  (66/66), 0 unresolved failures.** API Jest re-run in batches (a full-parallel run OOM'd on this
  Windows host): **1020/1024 executed tests pass**; the 4 failures are in
  `accounts-receivable.service.spec.ts` (compile-time DTO drift) and
  `client-onboarding.service.spec.ts` (incomplete Prisma mock) — both confirmed via `git log`/
  `git diff` untouched since an old BG milestone commit, unrelated to Supervisor, out of scope.
  **Two test-harness defects found and fixed** (both in `apps/web/e2e/` + `playwright.config.ts`,
  zero product-code changes): a session-race in the shared `uiLogin` fixture when a test switches
  roles mid-test without logging out first; a reservation-create test asserting nonexistent
  validation copy while also conflating native `<input type="date" min>` browser-level blocking
  with the app's own field validation. **Shared Neon `production` verified byte-for-byte
  unchanged** before/after (58 migrations/0 failed, 836 role_permissions, 126 reservations/12
  events, 1223 orders, 750 payments, 19 users, 6 discounts, 15 leave, 12 shift-swaps, 21
  anomalies; no sentinel tables leaked). Disposable Neon branch deleted; local Docker container
  removed; Prompt 4C recovery branch retained; Prompt 5A QA branch left to auto-expire on its own
  TTL. **No permission/schema/migration/seed/Postman/product-code change; no commit/push.**
  Reports: `ai/SUPERVISOR_RECONSTRUCTION_FINAL_COMPLETION_REPORT.md` (canonical),
  `ai/SUPERVISOR_FINAL_QA_EVIDENCE_INDEX.md`, `ai/SUPERVISOR_FINAL_KNOWN_LIMITATIONS.md`,
  `ai/SUPERVISOR_FINAL_DEMO_SCRIPT.md`, `ai/SUPERVISOR_FINAL_DEMO_DATA_REGISTER.md`. **Manager
  reconstruction is the next major track (not started).**

- SUPERVISOR_RECONSTRUCTION_PROMPT5B2 Approvals closure — Anomaly acknowledge/resolve + Shift-swap Outcome C — **B. COMPLETE WITH KNOWN LIMITATIONS / DEMO-READY — PROMPT 5 (SUPERVISOR APPROVALS) CLOSED** (2026-07-31): Completes the four-domain Approvals workspace (frontend-only — **no backend/schema/migration/seed/permission/Postman change; no commit/push**). **Anomaly:** `ApprovalAnomalyDetail` gains **Acknowledge** (`PATCH /analytics/anomalies/:id/acknowledge`, OPEN→ACKNOWLEDGED, note optional — the row **stays** in Needs action until resolved) + **Resolve** (`PATCH .../resolve`, ACKNOWLEDGED→RESOLVED, note **required**, confirm disabled until entered); both gated by `pos:analytics:anomalies:acknowledge`; the dialog states evidence is preserved and the underlying order/till/payment/attendance/shift record is not changed; acknowledge keeps the count, resolve decrements it. **Shift-swap = Outcome C (user-authorized after evidence review):** `ApprovalShiftSwapDetail` exposes **Reject only** (`PATCH /hr/shift-swaps/:id/approve {status:'REJECTED'}`, notes optional) with a truthful "Approval isn't available here — schedule reassignment is not supported" notice, and **no Approve control**. Rationale (decisive): **`ScheduleAssignment` is read-only across the ENTIRE API** (every reference is `findMany`/`count`/`findFirst` — no create/update/delete/reassign path; assignments are only seeded), `ShiftSwapRequest` carries only a bare `shiftDate` (no specific-shift FK → `findFirst` ambiguity), there is no role-compat/conflict/lock infrastructure, and `pos:hr:shift-swaps:approve` has **never** mutated the roster (SUP-RG-036 deferred it) — so a truthful atomic roster swap would be a net-new roster-write capability pulled forward from a deferred feature (§9 Outcome A unavailable). New decision fns in `lib/supervisor/approvals-workspace.ts` (`rejectShiftSwap`/`acknowledgeAnomaly`/`resolveAnomaly`); narrow invalidation (domain queues + counts + detail; anomaly acknowledge refetches detail keeping selection). **Validation:** web typecheck+lint+build pass (`/supervisor/approvals` 16.8 kB); API attendance+discounts+analytics+DTO **126/126** + reservations **39/39**. **Isolated live QA** on disposable Neon branch `br-hidden-king-a4rbwvj0` (fail-closed launcher, sentinel `P5B2-QA-20260731`, `/api/health` ok): **live API matrix 11/11** (shift-swap list/reject 200 + dup 400 + take=101 400; anomaly list/ack 200 + resolve-no-note 400 + resolve 200 + dup-resolve 400 + resolve-from-OPEN 400 + dup-ack 400) and **roster integrity — 0 `schedule_assignments` rows touched by a reject** (Outcome C truthful). **Full Playwright Approvals suite 120/120** (15 files × 4 viewports 1024×768/1366×768/1440×900/1920×1080, Chromium, `--retries=2`, 2 flaky recovered on transient compute stalls, exit 0) — anomaly ack/resolve, shift-swap reject + no-Approve notice, consolidated four-domain filters, cross-role (Cashier/Waiter no Approvals), responsive-closure, + full 5B1 regression. Anomaly specs deep-link to a seeded id by status to avoid severity-sort queue fragility. **Shared `production` verified UNCHANGED** before/after (58/836/126/1223/750/19; domain totals unchanged; **0 P5B2-QA rows; sentinel absent**); disposable branch deleted; Prompt 4C recovery branch retained; ports free; secret removed. **Supervisor Approvals CLOSED; Manager reconstruction NOT started.** Reports: `ai/SUPERVISOR_RECONSTRUCTION_PROMPT5B2_SHIFT_SWAP_ANOMALY_UI_COMPLETION_REPORT.md`, `ai/SUPERVISOR_RECONSTRUCTION_PROMPT5_APPROVALS_FINAL_COMPLETION_REPORT.md`, QA register + evidence index.
- SUPERVISOR_RECONSTRUCTION_PROMPT5B1 Approvals premium UI — Discount + Leave decisions — **B. COMPLETE WITH KNOWN LIMITATIONS / READY FOR PROMPT 5B2** (2026-07-30): The read-only Approvals page is replaced by `SupervisorApprovalsWorkspace` (`apps/web/src/components/supervisor/approvals/workspace/*`) built on the 5A `approvals-contract.ts` + a new additive `apps/web/src/lib/supervisor/approvals-workspace.ts` (normalisers, bounded fetchers, decision fns, canonical sort, scope/domain helpers). **Page:** **Needs action / Resolved / History** scope tabs (accessible `role=tablist`), All + per-domain filters (`aria-pressed`), server-`total` counts (permission-aware, no full-list count fetch), one identity-safe queue-row shell (domain badge, severity for anomaly, status, identity title, summary, actionable time, amount chip — never a raw UUID title), responsive master-detail (`xl:grid-cols-[minmax(0,1fr)_460px]`; stacks below xl; one detail workspace), URL-persisted `scope`/`domain`/`page`/`from`/`to`/`selDomain`/`selId` (default Needs action / All / page 1; **never** History; filter changes use `router.replace`, so Back returns to the prior page, not a filter state). Needs action = one bounded (≤100) fetch per domain (anomaly OPEN+ACK merged), merged for All, client-paginated 25/page; Resolved = recent 14-day window + client terminal filter; History = server-paginated + optional date range + client terminal filter (leave/swap/anomaly only). **Discount** approve/reject reuse the Prompt 3 `/pos/discounts/:id/approve|reject` + `order-financials` with the **UI-only payment-safety gate** (blocks approve when captured money exists) + truthful **self-approval notice** (backend permits it) + no optimistic totals; **Leave** approve/reject use `PATCH /hr/leave/:id/review` with **no payroll/roster claim**; terminal records read-only. **Shift-swap + Anomaly render READ-ONLY** (no decision controls — Prompt 5B2 activates them on the same architecture). Narrow invalidation via `approvalDecisionInvalidationKeys` (+ cross-role order-discounts/detail/payments/floor for discount approve); success via shared `ToastProvider` (`aria-live`). **Discounts omitted from Resolved/History** (no branch-wide endpoint, **SUP-RG-035**; truthful "available from the related order" notice if forced). **No permission / schema / migration / seed / backend / Postman change; no commit/push.** **Validation:** web typecheck+lint+build pass (`/supervisor/approvals` 16 kB); API attendance+discounts+analytics+DTO **126/126** + reservations regression **39/39**; `nest build` pass. **Isolated live browser QA:** disposable Neon branch `br-aged-resonance-a47lmtt5` (forked from `production`) via the Prompt 4D fail-closed launcher (denylist→preflight→spawn; sentinel `P5B1-QA-20260730`; `/api/health` `{status:ok,db:ok}`); pooled endpoint + keep-warm pinger stabilised the scale-to-zero compute. **Playwright Approvals suite 80/80** (10 files × 4 viewports 1024×768/1366×768/1440×900/1920×1080, Chromium, `--retries=2`) — discount approve/reject+self-approval, leave approve/reject, read-only shift-swap/anomaly, scopes/filters/counts, discount-history notice, URL state + refresh, identity/privacy, responsive no-overflow, Prompt 3/4 regression. **Shared `production` verified UNCHANGED** before/after (58 migrations/836 role_permissions/126 reservations/1223 orders/750 payments/19 users; pending counts identical; **0 P5B1-QA rows; sentinel absent**). QA discounts were seeded via SQL (not the order API) because of pre-existing `POST /pos/orders` order-number collision (**SUP-RG-040**, not a 5B1 defect); a "survive refresh" test assertion bug (goBack under replace-routing) was fixed. Disposable branch **deleted**; Prompt 4C recovery branch retained; ports free; secret removed. Prompt 5B2 **not started**. Reports: `ai/SUPERVISOR_RECONSTRUCTION_PROMPT5B1_APPROVALS_DISCOUNT_LEAVE_UI_COMPLETION_REPORT.md`, `ai/SUPERVISOR_APPROVALS_UI_QA_EVIDENCE_INDEX.md`, `ai/SUPERVISOR_APPROVALS_QA_RECORD_REGISTER.md`.
- SUPERVISOR_RECONSTRUCTION_PROMPT5A Approvals domain audit, lifecycle hardening, identity, queue contracts + isolated live QA — **A. COMPLETE / READY FOR PROMPT 5B** (2026-07-30): Backend/contract/QA foundation for the premium Approvals UI (5B). **No new permission, schema, migration, seed, or Postman change; no commit/push.** **Audit (code + live Neon, authoritative):** all four decision lifecycles already existed and pass Jest — Discount (`/pos/discounts/pending`+`:id/approve|reject`, `pos:discount:approve`), Leave (`/hr/leave` + `:id/review`, `pos:hr:leave:review`, in the **attendance** module), Shift-swap (`/hr/shift-swaps` + `:id/approve`, `pos:hr:shift-swaps:approve`), Anomaly (`/analytics/anomalies` + `:id/acknowledge|resolve`, `pos:analytics:anomalies:acknowledge`). Verified live on shared `production`: Supervisor holds all four decision perms + reads and does **NOT** hold `approvals:read`/`approvals:decide` → the generic `unified-approvals` inbox (`POST /api/approvals/:id/decide`) is deliberately **not** the Supervisor path → **domain-specific architecture (Option B)** locked. **Backward-compatible hardening applied (each with focused tests, existing suites still green):** (1) **pagination bounds** — leave/shift-swap list DTOs switched from unbounded `@IsNumberString` to coerced `@Type(()=>Number) @IsInt @Min @Max(100)` + service clamp; anomaly already bounded (SUP-RG-032 class fix, now pre-empted for all list DTOs). (2) **branch isolation** — shift-swap approve + anomaly ack/resolve lookups now filter `branchId` (were org-only → a same-org cross-branch mutation surface); **leave stays intentionally org-scoped** (nullable branch). (3) **concurrency** — all four decision writes now use a status-guarded conditional `updateMany` claim (loser of a race → 409 discount / 400–409 others; no duplicate mutation or audit). (4) **History** — optional `dateFrom`/`dateTo` added to leave/swap/anomaly list DTOs. (5) **identity** — anomaly **list** now includes a minimal `actorUser` projection (was detail-only). **Frontend (additive, page visually unchanged):** new `lib/supervisor/approvals-contract.ts` (domains, Needs-action/Resolved/History scopes over real statuses, canonical endpoints, bounded per-domain query builder, `ApprovalMinimalIdentity` + resolvers, query-key factory, counts-from-`total`, narrow decision invalidation, error mapping). **Documented contract gap for 5B:** discounts have **no branch-wide list endpoint** (only `/pending` + per-order) → discount Resolved/History is not a branch-wide queue without a new endpoint (out of scope). **Shared-Neon data audit (read-only, no PII):** `ai/SUPERVISOR_APPROVALS_SHARED_NEON_DATA_AUDIT.md` — queue counts, **all orphan/null-identity checks = 0** (clean identity data), 6 RESOLVED anomalies lack resolution notes + seeded-terminal rows carry no audit events (demo provenance, tolerate-don't-repair), **zero shared writes**. **Isolated live QA (full stack executed):** disposable Neon branch `br-polished-river-a4ep8bn0` (forked from production, auto-expiry) via the Prompt 4D fail-closed launcher (denylist→preflight→spawn, `/api/health` ok). **Live API decision matrix 29/29** (`tools/qa/approvals-live-matrix.mjs`: all four lifecycles + pagination-400 + History-window + required-reason + **branch-isolation 404** on same-org other-branch swap/anomaly + duplicate/concurrency **400/409** + identity names present). **Playwright Approvals smoke 8/8** across 4 viewports (route loads with hardened types, four-tab nav/no Orders, no console error; Prompt 3 Floor + Prompt 4 Reservations regression). **Shared `production` verified UNCHANGED** before/after (58 migrations/0 rolled back/836 role_permissions/126 reservations; pending counts identical; sentinel present only on disposable branch). Recovery branch `br-dawn-truth-a4zjs1p7` retained. **Validation:** web typecheck+lint+build pass; `nest build` pass; API tests attendance 47/47, discounts 22/22, analytics pass, DTO specs 35/35, reservations regression 39/39; Postman 56/56 valid; `git diff --check` clean. Prompt 5B UI **not started**. Reports: `ai/SUPERVISOR_RECONSTRUCTION_PROMPT5A_APPROVALS_LIFECYCLE_COMPLETION_REPORT.md`, `ai/SUPERVISOR_APPROVALS_SHARED_NEON_DATA_AUDIT.md`, `ai/SUPERVISOR_APPROVALS_QA_RECORD_REGISTER.md`.

- SUPERVISOR_RECONSTRUCTION_PROMPT4C shared-Neon migration cutover + QA closure — **COMPLETE WITH KNOWN LIMITATIONS / DEMO-READY** (2026-07-29): Controlled deployment pass. **After explicit user authorization**, deployed the Prompt 4A migration `20260518000000_prompt4a_reservation_completed_event` to the shared Neon **`production`** branch via **`db:migrate:deploy`** (`prisma migrate deploy`, on a constructed **direct** non-pooled connection — the repo `db:migrate` = `prisma migrate dev` is UNSAFE on shared and was NOT used). **Verified**: migration recorded (finished, not rolled back, steps=1), **checksum `8f1317fa…` matches the repo file**, enum `ReservationEventType` now has **COMPLETED** (after SEATED; all 9 prior values retained), 58 migrations total / 0 rolled back / 0 unfinished, reservation counts **unchanged** by the enum add (126). **`db:seed` applied** (authorized) → Supervisor now holds **`pos:order:transfer`** (role_permissions 835→836, exactly +1; all other counts unchanged) — **Transfer table (3B2) is now functional on shared Neon**, closing the 3C/3D residual. **CONSEQUENCE: manual "Mark visit complete" + automatic completion on order-close now WORK on shared Neon**, closing the 4B shared-Neon limitation — the full Supervisor reservations lifecycle is now demo-ready on shared. **Recovery branch** `br-dawn-truth-a4zjs1p7` (pre-migration snapshot) created before the write and **RETAINED** (Postgres can't drop an enum value → recovery = branch restore/forward-fix). **INCIDENT (full disclosure):** the optional isolated-API live-QA phase booted an API on :4002 that connected to **production** despite swapping `apps/api/.env` — root cause: a shell/profile-level `DATABASE_URL` overrode the file (`dotenv` never overrides an already-set env var; `ConfigModule.forRoot` had no `envFilePath`). The isolation check caught it after it created **one** marked QA reservation on production (`RES-000002`); the API was **killed within seconds**, `apps/api/.env` restored, and the row + its event **deleted with user authorization** → production **restored to exactly 126 reservations / 12 events / 0 QA rows**. Per user decision, closed at **B**; the live-API/browser four-viewport matrix was NOT run (isolation fragility + destructive-action classifier). Reservation lifecycle remains proven by **reservations+orders Jest 67/67** + the compiled 4B Playwright suite (72 tests × 4 viewports). **Shared-Neon net change: +1 migration, +1 role_permission; reservation/order/payment/user data UNCHANGED.** Disposable QA branch created→**deleted** (an earlier one auto-expired at the UTC date rollover). Validation: web typecheck/lint/build pass, Jest 67/67, Postman 56/56, `git diff --check` clean; **no code change in 4C**; `git` no commit/no push. Reports: `ai/SUPERVISOR_RECONSTRUCTION_PROMPT4C_SHARED_NEON_CUTOVER_COMPLETION_REPORT.md`, `ai/SUPERVISOR_RESERVATIONS_LIVE_QA_EVIDENCE_INDEX.md`, `ai/SUPERVISOR_RESERVATIONS_SHARED_NEON_DEPLOYMENT_READINESS.md` (now marked DEPLOYED). **Durable rule:** shared/production Prisma deploys use `db:migrate:deploy` only; `.env` swap alone does not isolate a Node process from an inherited shell `DATABASE_URL`.

- SUPERVISOR_RECONSTRUCTION_PROMPT4B Supervisor Reservations UI — **COMPLETE WITH KNOWN LIMITATIONS** (2026-07-28): Replaced the read-only **triple-query** Reservations page (all/today/upcoming @ `pageSize:100` + `mergeSupervisorReservationRows` browser merge) with a **premium master-detail workspace** on the Prompt 4A `scope=active|history` contracts. **Four UI views** (Arriving/Seated/Attention/History — groupings, NOT persisted statuses): Arriving/Seated/Attention derive from **ONE bounded `scope=active` query** (no browser merge, no triple fetch); **History** is a separate, lazy, server-paginated `scope=history` query. Default = Arriving + today + page 1 + bounded page size; **no all-history initial fetch; no pageSize-100 default** (active 50, history 25/max 100). **URL-persisted** view/date/page/status/from/to/selected (Back/Forward/refresh stable). **Lifecycle actions** wired to already-verified endpoints — **the Supervisor role already holds EVERY `pos:reservation:*` permission (seed Supervisor block), so NO permission change and NO backend change**: Create (`:create`), Confirm (`:confirm`), Assign/Change table (`:table:assign`), Seat (`:seat`, table required, **no order fabricated**), Cancel (`:cancel`, reason required), No-show (`:no-show`, **never for SEATED, never auto**), Manual complete (`:update` = `POST /:id/complete`, SEATED-only). **Action availability mirrors backend `VALID_TRANSITIONS` exactly** (PENDING→CONFIRMED/CANCELLED/NO_SHOW; CONFIRMED→SEATED/CANCELLED/NO_SHOW; SEATED→COMPLETED; terminal read-only). **Attention** derives overdue from server `overdue`/`overdueByMinutes` (grace 15m, PENDING/CONFIRMED only) + structural SEATED issues (no linked order / linked-order closed / no table); **operational copy only**, **individual actions only, NO bulk resolution**. **Automatic completion** presented truthfully (page never issues the mutation; order-close integration canonical). **Cross-role**: mutations persist canonically + **narrow invalidation** of Supervisor active/history/detail/events + Supervisor Floor overlay + Waiter reservations/floor — **never** menu/profile/auth/shift/approvals/all-orders/cashier. **Guest privacy**: names in rows, contact only in workspace/create form, synthetic QA only. **Deposit boundary**: create accepts optional `depositRequired` (verified DTO field); deposits read-only; **no payment/deposit capture**. **Files**: +7 components (`SupervisorReservationViewSelector/Row/DateToolbar/TableSelect/Workspace`, `SupervisorCreateReservationDialog`, `SupervisorReservationLifecycleDialogs`), lib `reservations.ts` extended (mutations/view-grouping/attention/action-availability/cache-invalidation/date-nav), page rewritten, barrel updated; **removed the 6 superseded read-only components** (Card/List/Summary/Toolbar/DetailPanel/StatusBadge); +Playwright `e2e/supervisor-reservations/` (9 specs). **Validation PASSED**: web typecheck + lint + `next build` (`/supervisor/reservations` 21.4 kB); **reservations+orders Jest 67/67**; Playwright **72 tests × 4 viewports compile** (`--list`); `git diff --check` clean; 56 Postman parse (0 Postman changes). **NEON MCP (read-only) verification**: project **nimbus-pos**, shared branch **`production`** — enum `ReservationEventType` still **LACKS `COMPLETED`**, migration `20260518000000_prompt4a_reservation_completed_event` **unapplied**; **shared Neon received ZERO writes** (2 read-only SELECTs). **Consequence → classification B**: **manual complete + auto-completion-on-order-close ERROR on shared Neon until that migration deploys; ALL other actions (create/confirm/assign/seat/cancel/no-show) + Attention/overdue WORK on shared today.** **QA-BLOCKED (not fabricated)**: live authenticated browser + 4-viewport + `/api/health` + disposable-branch mutation run need a running API/web/browser stack unavailable to this background agent. Full Approvals reconstruction **not started**. `git` no commit/no push. Reports: `ai/SUPERVISOR_RECONSTRUCTION_PROMPT4B_RESERVATIONS_UI_COMPLETION_REPORT.md`, `ai/SUPERVISOR_RESERVATIONS_SHARED_NEON_DEPLOYMENT_READINESS.md`, `ai/SUPERVISOR_RESERVATION_QA_RECORD_REGISTER.md`.

- SUPERVISOR_RECONSTRUCTION_PROMPT4A Neon reservation-lifecycle completion + active/history query repair + order-close sync — **COMPLETE WITH KNOWN LIMITATIONS / READY FOR PROMPT 4B (isolated Neon QA executed)** (2026-07-28): Repaired the reservation lifecycle + query architecture at the backend, the root cause of indefinite operational-list pile-up (not a frontend filter). **State machine** unchanged (`SEATED→COMPLETED` was defined-but-unreachable). **Manual completion:** new `POST /api/reservations/:id/complete` (`@HttpCode(200)`, `CompleteReservationDto` optional `note`) gated by **`pos:reservation:update`** — **already seeded AND already on Supervisor/Owner/Manager, so NO new permission and NO seed change** (§19 Outcome A; Waiter/Cashier intentionally cannot manually complete). Service `complete()` is SEATED-only, **idempotent** (already-COMPLETED returns canonical state, no 2nd event), atomic, emits `ReservationEvent(COMPLETED,{source:'manual'})` + `RESERVATION_COMPLETED` audit; supports completion **with or without a linked order** (no fabricated order/payment). **Auto-completion on order close:** `ReservationsService.completeForClosedOrder()` invoked from the single canonical choke point `OrdersService.transitionOrder()` (`targetStatus===CLOSED`, after committed status change + table release, **never** in Cashier FE); keys **only** off explicit `seatedOrderId + status=SEATED` (never table/guest/date inference); retry-safe/idempotent (conditional update → no duplicate event); **after-close reconciliation** (existing close is non-transactional) — order close stays canonical, completion failure is **logged not swallowed** and never rolls back the close. **Concurrency:** all transitions (confirm/seat/cancel/no-show/complete) now use a guarded conditional `updateMany({where:{id,status:{in:[from]}}})` compare-and-set → stale requests 409, no lost update, no duplicate events. **Active/History query split:** `list()` gains `scope=active|history` (server-side status grouping), `from`/`to` range, deterministic sort (history desc, else asc, id tiebreak), **pageSize default 25 clamped to max 100** (service clamp, not 400-reject, to avoid breaking legacy callers), returns `{data,total,page,pageSize,totalPages,scope}`; server-derived `overdue`/`overdueByMinutes` (never persisted; grace `OVERDUE_GRACE_MINUTES=15`; **never auto-NO_SHOW**); `seatedOrder{id,orderNumber,status}` included for 4B Attention derivation. **Timezone:** UTC day edges (branch tz not modelled — documented). **DTO:** `ListReservationsQueryDto` +`scope`/`from`/`to`, kept `@Type(()=>Number)` coercion. **Schema/migration:** ONLY change = `COMPLETED` added to `ReservationEventType` enum (after `SEATED`) + migration `20260518000000_prompt4a_reservation_completed_event` (`ALTER TYPE … ADD VALUE`); Prisma client regenerated (codegen only); **NOT deployed to shared Neon this pass** (§44); no index migration needed (existing `[branchId,status]`/`[branchId,reservationAt]` cover it). No circular DI (OrdersModule imports ReservationsModule; reservations doesn't import orders). **Frontend (contract only, NO UI redesign):** `lib/supervisor/reservations.ts` +`scope`/`from`/`to` query, +`totalPages`/`scope`/`overdue` types, `fetchSupervisorActiveReservations`/`fetchSupervisorReservationHistory`/`completeSupervisorReservation`, `supervisorReservationKeys` factory + scoped `supervisorReservationInvalidationKeys()` (never invalidates menu/profile/auth/shift/approvals/all-orders/cashier); legacy triple-query merge left intact but no longer the target contract (Prompt 4B consumes scopes directly). **Validation PASSED:** API typecheck clean for changed modules (4 remaining errors are pre-existing in `accounts-receivable.service.spec.ts`, untouched); **reservations + orders Jest 67/67 pass** (incl. complete/idempotency/race/order-close/concurrency/scope/clamp/overdue); web typecheck + lint + **`next build` all pass**; all 56 Postman collections parse (M16 extended: Complete + active/history scope + pageSize-clamp requests). **ISOLATED NEON QA EXECUTED (2026-07-28, via Neon MCP):** project **nimbus-pos** (`empty-glade-…`); only branch is `production` (default/primary, live app DB) → classified **shared/production, read-only for destructive QA**; disposable fork `prompt4a-reservations-qa-20260728` (`br-weathered-breeze-…`) created + isolation proven (forked 126 res/57 migrations, distinct compute). **Migration audit:** shared 57 applied/0 failed/latest bg7, repo 58, only the 4A migration unapplied (intended), no drift. **Migration applied on QA branch:** `COMPLETED` now at enum ord 5.5 (after SEATED); shared enum untouched. **Permissions (live SQL):** Supervisor HAS `pos:reservation:update` (1 perm row, 0 dup role-perms). **Data audit (shared, read-only, no PII):** 126 reservations — 9 PENDING/52 CONFIRMED/6 SEATED (all order-less, past)/57 COMPLETED/1 CANCELLED/1 NO_SHOW; 55 overdue actives; 0 completed-with-open-order, 0 cross-branch links. **Live lifecycle on QA branch:** manual complete SEATED→COMPLETED (1 row + `{source:manual}` event; idempotent re-run 0 rows); order-close auto-complete on a linked SEATED+CLOSED fixture (1 row + `{source:order-close,orderId}` event; duplicate close 0 rows; unlinked close 0 rows); active/history split verified (65+61=126, no overlap); query plans ≈0.07ms (seq-scan optimal at 126 rows, existing indexes cover growth → no index migration). **Shared Neon received ZERO writes** (before/after counts identical: 126 res, 12 events; enum still lacks COMPLETED). **6 order-less SEATED + 55 overdue actives = SUPERVISOR-DECISION-REQUIRED repair candidates (NOT auto-resolved; no auto-NO_SHOW).** → **B. COMPLETE WITH KNOWN LIMITATIONS / READY FOR PROMPT 4B.** **Remaining non-blocking depth (not run):** HTTP-layer API boot + isolated `/api/health` + Playwright smoke (controller/DTO/permission stack unit-tested 67/67, DB contract proven live). No mass shared-Neon data repair (dry-run-first plan in the audit doc). Disposable branch auto-expires 2026-07-29T02:00Z; explicit delete pending user OK. `git` no commit/no push. Reports: `ai/SUPERVISOR_RECONSTRUCTION_PROMPT4A_NEON_RESERVATION_LIFECYCLE_COMPLETION_REPORT.md`, `ai/SUPERVISOR_RESERVATION_QA_RECORD_REGISTER.md`, `ai/SUPERVISOR_RESERVATION_SHARED_NEON_DATA_AUDIT.md`.

- SUPERVISOR_RECONSTRUCTION_PROMPT3D isolated destructive QA + browser matrix + Prompt 3 closure — **COMPLETE WITH KNOWN LIMITATIONS (Prompt 3 = demo-ready with documented boundaries)** (2026-07-28): Closed the QA debt from 3C using the user's explicit authorization to stand up an **isolated disposable stack** — Docker **Postgres 16** (`nimbus_prompt3_qa`, host 55432; cinemax containers untouched), fresh API on **:4001**, built web on **:3100** — applied all migrations + `db:seed` + `db:demo:import`, installed **Playwright + Chromium**, and ran the full destructive matrix **against the disposable DB only (shared Neon received ZERO writes**; isolated API booted with no Neon cold-start, proving local DB). On the isolated DB Supervisor holds all 10 grants **incl. `pos:order:transfer` (HAS_TRANSFER=true)** so Transfer table works at runtime. **API mutation matrix (41 checks): all in-scope actions + rejection cases + idempotency replays PASS** — request-bill (dup-safe, terminal-reject), mark-served (READY→SERVED via send→in-kitchen→ready; NEW→409), split-bill EQUAL/CUSTOM (allocated==total; mismatch 400; idempotent replay), split-items (child created; idempotent same child; qty-too-high 400), move-items (same-order 400; idempotent), merge (source VOIDED; self-merge 400), transfer-table (200; same-table 400; idempotent; VOIDED-source 409), lookup (exact/bounded-25/missing-404/tableless), void (VOIDED; repeat 409), discount %/FIXED (auto-approve ≤threshold 150000; value-0/150/over-subtotal validation), PENDING>threshold→approve (totals recalc; dup-approve 409)/reject (totals unchanged; missing-reason 400), self-approval (backend-permitted), complimentary (metadata round-trips; total floors at 0). **Financial integrity: no total ever negative** (over-subtotal FIXED floors at 0). **DEFECT FOUND & FIXED (backend, proven by live QA):** `GET /pos/orders/:id/discounts?pageSize=N` returned **400** (query `page`/`pageSize` not coerced — `ListOrderDiscountsQueryDto` lacked `@Type(() => Number)`); this is the exact read the Supervisor **Discounts panel** uses (`?pageSize=50`), so the panel read was broken at runtime. **Fix:** added `@Type(() => Number)` (mirrors `list-orders-query.dto.ts`) — now 200, complimentary metadata round-trips; **focused Jest DTO spec added (6 tests, pass)**. No contract change → **Postman unchanged**; no schema/migration/permission/role change. **Browser QA: Playwright 64/64 PASS across all four viewports** (1024×768/1366×768/1440×900/1920×1080) — auth+Floor, 4-tab nav (no Orders), Find order, workspace action availability (no payment/close/refund controls), Request bill through the UI, legacy `/supervisor/orders` redirect, role boundaries (Waiter/Cashier isolated; Waiter blocked from Supervisor workspace), no-horizontal-overflow at every viewport, Waiter/Cashier regression. **Transfer safety (§12):** confirmed backend moves `order.tableId` only (no occupancy/reservation validation); the UI warns-not-blocks (documented 3B2 design; recoverable, no data corruption) — no code change. web typecheck/lint/build pass; `git diff --check` clean; Postman 53/56 valid (+3 pre-existing BOM). **Playwright harness kept** (`apps/web/playwright.config.ts` + `e2e/supervisor-prompt3/*`, env-driven, artifacts git-ignored). Cleanup: isolated API/web stopped, disposable DB container removed, no secrets persisted. **Residual non-blocking limitations:** UI-only payment safety (backend doesn't self-check), backend self-approval permitted, `pos:order:transfer` gates transfer-server (API-reachable, no UI), transfer-table warns-not-blocks, and **shared Neon still needs `db:seed` to apply the transfer mapping before a Neon demo**. `git` no commit/no push. Reports: `ai/SUPERVISOR_RECONSTRUCTION_PROMPT3D_ISOLATED_QA_COMPLETION_REPORT.md`, `ai/SUPERVISOR_PROMPT3_QA_RECORD_REGISTER.md`.

- SUPERVISOR_RECONSTRUCTION_PROMPT3C consolidated live-QA / closure pass — **IMPLEMENTED / QA BLOCKED** (2026-07-28): Verification-only closure pass (no new Supervisor capabilities, no code changes). **Executed & PASSED:** worktree safety confirmed (branch `main`, 12 intentional Floor deletions present, all 3B1–3B3B dialog files present, no stray processes/ports); web `typecheck`/`lint`/`build` all exit 0; `git diff --check` clean; Postman 53/56 JSON valid (3 pre-existing UTF-8 BOM in M17/M18/M19, unmodified vs HEAD, out of scope; the 4 Prompt-3 collections valid); API built fresh + booted, **`/api/health` = `{status:ok, db:ok}`**; API `orders.service.spec.ts` **26/26 pass**. **Live read-only runtime permission verification (§10):** Supervisor login **201**, `/auth/me` 132 perms; guard-boundary probes on nonexistent ids returned **non-403** (400/404) for request-bill/mark-served/split-bill/split-items/move-items/merge/void/discount request/approve/reject — proving each in-scope permission is GRANTED with **zero mutation**. **Code-level locked decisions re-verified:** Supervisor nav = Floor/Reservations/Approvals/Me (no Orders tab); `/supervisor/orders` = legacy redirect via the shared order-detail key (no loop/dup fetch); **transfer-server has NO UI** (registry `blockedReason` only); Find order uses the shared `OperationalFloor` (never forks); payment safety gate (`requiresCleanPayment` on void/discount/approve/complimentary, fail-closed on `paymentUnavailable`, reject not gated); action registry has exactly the 13 in-scope keys and **zero forbidden actions** (no payment-collect/order-close/refund/post-close-void). **Defect / actionable gap found:** the Supervisor role **LACKS `pos:order:transfer` on the active Neon DB** — confirmed via `/auth/me` (LACKS), a direct `role_permissions` read, AND a live **403** on `POST …/transfer-table`; **Transfer table (3B2) is non-functional at runtime until the authorised additive seed mapping is applied** (run `db:seed`, or insert the single roleId `cmqlcft890006wp6loken0xub` × `pos:order:transfer` row; no schema/migration — the 3B2 seed diff was never re-applied to this DB, consistent with that report's "re-seed required to apply"). **NOT executed / QA-blocked (not fabricated):** destructive live-mutation QA §11–§23 (shared live Neon DB + the environment's safety classifier blocked DB writes; no isolated test DB) and authenticated browser + four-viewport QA §25–§27 (no Playwright/Puppeteer/Cypress installed). Per the prompt, **DEMO-READY (A) not claimed** on static gates alone → status **C. IMPLEMENTED / QA BLOCKED**. No backend/API/DTO/Prisma/migration/seed/permission/Postman change; **no DB mutation, no commit, no push**. Reports: `ai/SUPERVISOR_RECONSTRUCTION_PROMPT3_CONSOLIDATED_LIVE_QA_COMPLETION_REPORT.md`, `ai/SUPERVISOR_PROMPT3_QA_RECORD_REGISTER.md`.

- SUPERVISOR_RECONSTRUCTION_PROMPT3B3B discount approve/reject + complimentary complete / Supervisor order-workspace financial actions feature-complete (2026-07-28): Implemented the discount decision lifecycle + whole-order complimentary on the 3B3A foundation. **No permission change and no backend change** — Supervisor already held `pos:discount:approve` (gates BOTH approve + reject), `pos:discount:request`, `pos:discount:read`, so no Section-6 stop condition (seed.ts unchanged this prompt). **Approve** (`POST /pos/discounts/:id/approve`, `pos:discount:approve`, HTTP 200, NOT BG3): PENDING-only + order must stay discountable (NEW/SENT/IN_KITCHEN/READY, SERVED/terminal → 409); managerPin OPTIONAL (re-auths the approver's own quick-PIN, not required — UI does not collect it); recalcs totals (latest approved wins, tax ignored); bare response → re-fetch. **Reject** (`POST /pos/discounts/:id/reject`, same permission, 200): `rejectionReason` REQUIRED; PENDING-only; **does NOT change totals**. Both rendered as **inline Approve/Reject controls on PENDING discount rows** in the read-only Discounts panel (shown only with `pos:discount:approve` + order-level availability). **Self-approval: backend PERMITS it (no guard); the UI matches + surfaces a truthful "You requested this discount" note** — a backend self-approval/maker-checker guard is recommended (governance). **Complimentary = Outcome B** (no dedicated type, but `Discount.metadata` persists AND round-trips on all reads): a whole-order `PERCENTAGE value=100` discount request + `metadata {complimentary,category}` + required reason, in the **Adjustments** group with a constrained category select; whole-order only (no line-level backend support); may return PENDING above the org threshold (default 5000); distinct from Void and Refund; no optimistic zero total. **Payment safety gate** (UI-only, from 3B3A) extended to approve + complimentary (both mutate totals); reject is NOT payment-gated (non-mutating). **Narrow invalidation**: approve/reject/complimentary touch only order-discounts + order-detail (approve/comp only) + Floor (approve/comp only) + `["supervisor","approvals","discounts",branchId]` + the approval-detail key — never leave/shift-swap/anomaly/reservation/profile/auth/shift; the Approvals page keeps its read-only layout (not redesigned). Added pure helpers (`order-financials.ts`: complimentary builder + 5 categories, manager-PIN validation, approve/reject error copy) + `prompt3b3b-assertions.ts`; updated `prompt3a`/`prompt3b1`/`prompt3b3a` assertions (approve/reject/complimentary left the hidden sets; superseded 3B3A structural checks). Verified: web typecheck, lint (no warnings), production build, floor/shell/profile/prompt3a/prompt3b1/prompt3b2/prompt3b3a/prompt3b3b assertions all pass, `git diff --check` clean, 56/56 Postman JSON valid (no Postman change — M12 already covers approve+reject). **Consolidated live `/api/health` + authenticated + browser/viewport QA for Prompts 3B1–3B3B remains PENDING** — no API/DB/browser automation (API not on :3001); NOT fabricated. Supervisor order-workspace financial actions are now feature-complete; transfer-server/refund/post-close-void/payment/close stay out of scope. `git` no commit/no push. Report: `ai/SUPERVISOR_RECONSTRUCTION_PROMPT3B3B_DISCOUNT_DECISIONS_AND_COMPLIMENTARY_COMPLETION_REPORT.md`.

- SUPERVISOR_RECONSTRUCTION_PROMPT3B3A active-order void + order-level discount request complete / Prompt 3B3B not started (2026-07-28): Implemented the next verified financial-adjustment capabilities on the 3A/3B1/3B2 foundation. **No permission change and no backend change** — Supervisor already held `pos:orders:void`, `pos:discount:request`, `pos:discount:read`, `pos:discount:approve`, so there was no Section 6 stop condition (the `seed.ts` diff in the worktree is the earlier 3B2 `pos:order:transfer` grant, not 3B3A). **Active-order void** (`POST /pos/orders/:id/void`, `pos:orders:void`, HTTP 200, NOT BG3): reason required in the UI (backend requires it for IN_KITCHEN/READY), no manager PIN; allowed statuses NEW/SENT/IN_KITCHEN/READY (SERVED→409, CLOSED/VOIDED→409); backend sets status=VOIDED only and auto-releases an idle DINE_IN table; canonical merge into order-detail + removal from Floor activeOrders (source card frees) + Floor/Waiter-Floor invalidate; workspace stays on the read-only voided order. Void is explicitly separated from refund, complimentary, and post-close void in code/UI/docs. **Order-level discount request** (`POST /pos/orders/:id/discounts`, `pos:discount:request`, HTTP 201, NOT BG3): type PERCENTAGE(0.01–100)/FIXED(≥0.01, ≤subtotal), reason required, basis = subtotal; the backend AUTO-APPROVES when the amount is within the org threshold (default 5000) else returns PENDING (amount-based, NOT permission-based) — the UI shows an estimate and defers the final status to the response (no optimistic final total; re-fetches order-detail for canonical totals). New read-only **Discounts** panel in the workspace (type/value/status/reason/requester/created/reviewer; no approve/reject controls). **Payment safety gate (documented, UI-only):** because the backend does NOT itself check payment on either endpoint, the availability module blocks void + discount when payment state indicates money (settled/partially-paid/pending/failed/refunded) or can't be confirmed (loading/errored) — never assumes "unpaid" on a failed read. Narrow invalidation only: discount request touches order-detail + order-discounts + (approved) Floor + the `["supervisor","approvals","discounts",branchId]` count — never leave/shift-swap/anomaly/reservation/profile/auth/shift. A second discount is UI-blocked while a PENDING one exists (backend allows multiples; UI guards duplicates). Added pure helpers (`lib/supervisor/order-financials.ts`: percentage/fixed validators, backend-mirrored preview math, reason validation, error mapping) + `prompt3b3a-assertions.ts`; updated `prompt3a`/`prompt3b1` assertions (void + request-discount left the "stays hidden" sets). Verified: `corepack pnpm@8.15.0` 8.15.0, web typecheck, lint (no warnings), production build, floor/shell/profile/prompt3a/prompt3b1/prompt3b2/prompt3b3a assertions all pass, `git diff --check` clean (only pre-existing LF→CRLF on untouched files), 56/56 Postman JSON valid (BOM-tolerant; no Postman change). **Authenticated `/api/health`, live void/discount QA, the Jest API suite, and browser/viewport QA are PENDING** — no API/DB/browser automation available (API not listening on :3001); NOT fabricated. Prompt 3B1/3B2 browser QA also remains pending. approve-discount/reject-discount/complimentary/refund/post-close-void/transfer-server stay hidden (Prompt 3B3B and later). `git` no commit/no push. Report: `ai/SUPERVISOR_RECONSTRUCTION_PROMPT3B3A_VOID_AND_DISCOUNT_REQUEST_COMPLETION_REPORT.md`.

- SUPERVISOR_RECONSTRUCTION_PROMPT3B2 transfer table + Find order + tableless/terminal/legacy lookup complete / Prompt 3B3 not started (2026-07-28): Implemented the next controlled Supervisor capability group on the Prompt 3A/3B1 foundation. **RBAC change (user-authorized via AskUserQuestion — chose "Grant + enable transfer-table", overriding the prompt's no-permission-change rule):** the Supervisor role was granted `pos:order:transfer` by adding it to its permission array in `packages/db/prisma/seed.ts` (no schema/migration; re-seed required to apply). ⚠️ `pos:order:transfer` is a SINGLE backend gate covering BOTH `POST /pos/orders/:id/transfer-table` AND `.../transfer-server`; only transfer-table has UI — transfer-server stays deferred (no safe branch-scoped server selector; only admin-gated tenancy memberships or PII/payroll-leaking HR/workforce lists exist — Outcome B), so the transfer-server endpoint becomes API-reachable for Supervisor but has no UI (audit-logged, active-same-branch-membership required). **Transfer table** (POST `/pos/orders/:id/transfer-table`, HTTP 200, BG3 optional idempotency): bounded branch-scoped target-table selector reusing the Floor query (excludes the current table; occupied/reserved surfaced as honest non-blocking warnings because the backend only moves `order.tableId` and does NOT validate occupancy/reservation/capacity or change table status); `Idempotency-Key` attached; canonical order-detail + Supervisor Floor caches reassigned to the returned table (source card frees, target card fills) + Waiter Floor invalidated; on success the workspace re-anchors to the returned table (selection + URL `tableId`/`orderId` via `router.replace`, orderId preserved, no redirect loop, refresh-stable). **Find order** compact Supervisor-only Floor control (NOT an Orders tab; injected as a sibling above the shared `OperationalFloor`, never forked): bounded/paginated branch lookup (25 rows), status/service filters, client-side text filter, and an exact-order-ID direct `GET /pos/orders/:id` fallback (backend has NO order-number search — documented). Opens takeaway/tableless/closed/voided/exception orders in the canonical workspace; **tableless** orders open with `orderId` only (no fabricated table); **terminal** orders render read-only with handoff/service actions disabled-with-reason. Legacy `/supervisor/orders` (+`tableId`/`orderId`/both) redirect-into-Floor verified across all four paths (no visible Orders nav, no redirect loop, no duplicate order-detail fetch — shared query key). Central availability extended: `transfer-table` live; `transfer-server` stays foundation (blockedReason). Added pure helpers (`lib/supervisor/transfer-table.ts`: target derivation/warnings, submission validity, error-copy mapping) + `prompt3b2-assertions.ts`; updated `prompt3b1-assertions.ts` (transfer-table moved out of the "stays hidden" set). Verified: `corepack pnpm@8.15.0` 8.15.0, web typecheck, lint (no warnings), production build, floor/shell/profile/prompt3a/prompt3b1/prompt3b2 assertions all pass, `git diff --check` clean (only pre-existing LF→CRLF warnings on untouched files), 56/56 Postman JSON valid (BOM-tolerant; 3 pre-existing BOMs; no Postman change — transfer contract unchanged, BG4B already covers it). Only backend change is the `seed.ts` role mapping. **Authenticated `/api/health`, live Supervisor transfer/lookup QA, browser/viewport QA, and the Jest API suite are PENDING** — no API/DB/browser automation is available in this environment (API not listening on :3001); NOT fabricated. Prompt 3B1 browser QA also remains pending. Prompt 3B3 (transfer server, active/post-close void, discount request/approve/reject, complimentary, refunds) NOT started. `git` no commit/no push. Report: `ai/SUPERVISOR_RECONSTRUCTION_PROMPT3B2_TRANSFER_AND_LOOKUP_COMPLETION_REPORT.md`.

- SUPERVISOR_RECONSTRUCTION_PROMPT3B1 split / split-items / move / merge complete / Prompt 3B2+3B3 not started (2026-07-27): Implemented the four Supervisor order-handoff actions inside the Floor table workspace on the Prompt 3A foundation. **RBAC change (user-authorized, overriding the prompt's no-permission-change rule):** the Supervisor role was granted `pos:order:split`, `pos:order:merge`, `pos:order:move-items` by mapping it to the already-defined permission rows in `packages/db/prisma/seed.ts` (no schema/migration change; re-seed applied idempotently — verified the three RolePermission rows added and the endpoints now return 400 validation instead of 403). Extended the central action-availability module (`lib/supervisor/order-actions.ts`) so split-bill/split-items/move-items/merge are live (permission + open-status + line/total gating), while transfer-table/transfer-server/void/discount/complimentary stay hidden for 3B2/3B3. Added pure validators (`lib/supervisor/order-action-forms.ts`: EQUAL/CUSTOM split allocation in integer cents mirroring the backend, line-selection quantity validation, body builders), a bounded branch-scoped order-target selector (excludes CLOSED/VOIDED + the source order, paginated, searchable), a shared line-selection primitive, and four dialogs reusing the extended `ActionConfirmDialog` (composable `children` + `confirmDisabled` + `size`) and the idempotency-intent utility (`Idempotency-Key` attached — these endpoints ARE BG3-wrapped). Split bill is non-physical (records payable allocation groups on order metadata; no new orders, no payment collected); Split items creates a NEW child order; Move items updates source+target; Merge voids the source into the surviving target and the workspace navigates to it. Narrow canonical order-detail + Floor cache updates; payment stays read-only; no visible Orders nav; shared Floor parity intact. Verified: web typecheck, lint (no warnings), production build, floor/shell/profile/prompt3a/prompt3b1 assertions, `/api/health` db ok, and authenticated supervisor endpoint QA — split-bill EQUAL 200 (3 groups, allocated==total) + idempotent replay + CUSTOM-mismatch 400; split-items 200 (child ORD-…-S1 NEW) + idempotent replay same child; move-items 200 (source/target updated) + same-target 400; merge 200 (source VOIDED, mergedIntoOrderId=target) + self-merge 400. Waiter/Cashier login 201 + `/auth/me` 200 (no regression). Postman unchanged (HTTP contracts unchanged; 56/56 JSON valid); only backend change is `seed.ts` role mapping; `git diff --check` clean; no commit or push. QA-created data: 1 split-bill metadata record, 1 child split order, 1 move between two orders, 1 merged (voided) source. Report: `ai/SUPERVISOR_RECONSTRUCTION_PROMPT3B1_SPLIT_MOVE_MERGE_COMPLETION_REPORT.md`. Authenticated browser/viewport QA still pending (no browser automation in this environment).

- SUPERVISOR_RECONSTRUCTION_PROMPT3A action foundation + safe service actions complete / Prompt 3B not started (2026-07-27): Established the reliable foundation for Supervisor order actions and shipped the two verified, non-financial service actions. Closed the Supervisor idle-session parity gap by reusing the shared `OperationalIdleLogoutHandler` (SupervisorShell now injects it; idle constants moved to `@/components/pos-shell/idle` as `OPERATIONAL_IDLE_TIMEOUT_MS` (15 min preserved) / `OPERATIONAL_ACTIVITY_EVENTS`, with back-compat aliases in `lib/waiter/idle`; one timer per shell, no duplicate logout/`/auth/me`). Added a central action-availability module (`lib/supervisor/order-actions.ts`) that is the single source of truth for visible/enabled/reason/requiresConfirmation/requiresReason/requiresManagerPin/requiresIdempotencyKey across all 13 order actions; only `request-bill` and `mark-served` are live in 3A, the rest are hidden foundation for 3B. Added a shared composable confirmation dialog (`components/pos-shell/ActionConfirmDialog.tsx`, focus/Escape/validation/pending/error/focus-return, no window.confirm) and an idempotency-intent utility (`lib/pos-shell/idempotency.ts`, generate-on-submit/reuse-on-retry/clear-on-success-or-change) as prepared foundation — intentionally NOT attached to request-bill/mark-served because those backend endpoints are not BG3-wrapped and do not honor `Idempotency-Key`. Implemented **Request bill** (POST `/api/pos/orders/:id/request-bill`, no body, audit-only, duplicate-safe, session-scoped truthful acknowledgment) and **Mark served** (POST `/api/pos/orders/:id/mark-served`, READY→SERVED, explicit confirmation, optional reason, narrow canonical order-detail + floor cache updates). Payment state remains read-only; tableless order context preserved; no Orders nav reintroduced; shared Floor parity intact. transfer-server stays deferred (no safe narrow server selector; only admin-gated/unfiltered tenancy memberships or org-wide HR exist). Verified: web typecheck, lint (no warnings), production build, floor/shell/profile/prompt3a assertions, `/api/health` db ok, and authenticated supervisor endpoint QA — request-bill 200 (billRequested true) + duplicate 200 + 409 on CLOSED; mark-served 409 on NEW (invalid transition) + 200 READY→SERVED verified by GET; Waiter/Cashier login 201 + `/auth/me` 200 (no regression). Backend/API/DTO/Prisma/migration/seed/permissions/Postman contracts unchanged (56/56 Postman JSON valid); `git diff --check` clean; no commit or push. QA-created data: one demo order transitioned READY→SERVED. Report: `ai/SUPERVISOR_RECONSTRUCTION_PROMPT3A_ACTION_FOUNDATION_COMPLETION_REPORT.md`.

- CLAUDE_REPOSITORY_ONBOARDING_AND_UI_VERIFICATION complete / documentation consolidation + safe polish (2026-07-26): Repository onboarding and continuity pass by Claude (Opus 4.8). Confirmed canonical path `C:\Users\arman\Desktop\nimbus-pos`, dirty `main` worktree preserved (no reset/restore/stash/clean; no commit/push). Verified in code that Waiter nav = Floor/Reservations/Me, Cashier = Queue/Receipts/Till/Me, Supervisor = Floor/Reservations/Approvals/Me with NO visible Orders tab, shared `OperationalShell`/`OperationalFloor`/profile primitives, and legacy `/waiter/orders*` + `/supervisor/orders` redirect-only. Confirmed Supervisor Reconstruction Prompt 0/1/2 complete and Prompt 3 NOT started. Created root `CLAUDE.md`, `PROGRESS.md`, `ARCHITECTURE.md`; created `docs/DOCUMENT_INDEX.md`, `docs/REPOSITORY_MAP.md`, `docs/UI_SYSTEM.md`, `docs/ROLE_JOURNEYS.md`, `docs/ROLE_CAPABILITY_MATRIX.md`, `docs/DECISIONS.md`, `docs/TESTING_AND_QA.md`, `docs/KNOWN_LIMITATIONS.md`; added supersession banners to the legacy 5-tab Supervisor pack docs under `Front End/supervisor_ui_docs_pack/docs/supervisor-ui-docs/`. One safe UI polish: `OperationalTableCard` capacity footer now matches its accessible label (was visible `"? seats"` vs aria "seat capacity unavailable"). Documented (not patched) the Supervisor idle-logout omission and cross-role idle-constant naming. Validation passed: `corepack pnpm@8.15.0` 8.15.0, web typecheck, web lint (no warnings), web production build, floor/shell/profile static assertions, `/api/health` db ok, login 201 + `/api/auth/me` 200 for Waiter/Cashier/Supervisor, `git diff --check` clean, all 56 Postman JSON valid (3 legacy UTF-8 BOM tolerated by Postman). No backend/API/DTO/Prisma/migration/seed/permission/auth/Postman contract changed; no commit or push. Report: `ai/CLAUDE_REPOSITORY_ONBOARDING_AND_UI_VERIFICATION_REPORT.md`.

- SUPERVISOR_RECONSTRUCTION_PROMPT2 shared operational Floor complete (2026-07-18): Waiter and Supervisor now consume one shared `OperationalFloor` tree, toolbar, grid, table card, status badge, state treatment, and responsive workspace frame. Their role-owned queries normalize into one presentation contract; Waiter still opens its instant menu/order-entry workspace, while Supervisor opens a read-first table-control workspace with table, active-order lines/totals/age, bill, payment, reservation, and verified table-status context. Supervisor selection is URL-backed with `tableId`/`orderId`, Back/Forward and focus return work, and legacy `/supervisor/orders` context resolves into Floor without a visible Orders tab. High-impact order actions remain deferred. Typecheck, lint, production build, focused assertions, API health, authenticated cross-role browser QA, four-viewport geometry checks, and legacy-order resolution passed. Postman/backend/Prisma/migrations/seed were unchanged; no commit or push occurred. Report: `ai/SUPERVISOR_RECONSTRUCTION_PROMPT2_SHARED_FLOOR_COMPLETION_REPORT.md`.

- SUPERVISOR_RECONSTRUCTION_PROMPT1 shared shell complete / Floor reconstruction pending (2026-07-18): Waiter, Cashier, and Supervisor now consume one `OperationalShell`, `OperationalHeader`, `OperationalBottomNav`, shared `CurrentTime`, and canonical operational icon registry while their guards, readiness queries, navigation labels, and business workflows remain role-owned. Waiter navigation remains Floor/Reservations/Me; Cashier remains Queue/Receipts/Till/Me; Supervisor visible navigation is now exactly Floor/Reservations/Approvals/Me. `/supervisor/orders` is legacy-only and safely redirects to Floor, preserving `tableId`; `orderId` is resolved through the existing detail endpoint to its table when available and otherwise remains truthful Floor lookup context. Focused shell assertions, web typecheck/lint/production build, API health, authenticated role/guard/logout/legacy-route QA, request-count inspection, and all four target viewports passed. Postman remained unchanged; no backend/API/DTO/Prisma/migration/seed/permission/auth/cache-key contract changed; no commit or push occurred. Report: `ai/SUPERVISOR_RECONSTRUCTION_PROMPT1_SHARED_SHELL_COMPLETION_REPORT.md`.

- SUPERVISOR_RECONSTRUCTION_PROMPT0 complete / implementation pending (2026-07-18): Supervisor reconstruction repo verification, shared-component mapping, lifecycle audit, and phased roadmap completed as a documentation-only pass. Verified that the new target visible Supervisor nav is exactly Floor, Reservations, Approvals, Me; Orders must be removed from visible nav and replaced by Floor-contained table/order workspace plus exception lookup for tableless, takeaway, closed, direct-reference, and post-close review cases. Verified current divergence between Waiter Floor and Supervisor Floor, documented the shared operational floor extraction path, mapped shell/header/nav/icon reuse across Waiter/Cashier/Supervisor, and documented reservation and approval pile-up root causes. No runtime code, backend/API/DTO/Prisma/migration/seed/Postman/permission/auth/routing/React Query/cache behavior, commits, or pushes were changed. Reports/docs: `ai/SUPERVISOR_RECONSTRUCTION_REPO_VERIFICATION_REPORT.md`, `ai/SUPERVISOR_RECONSTRUCTION_ROADMAP.md`, `ai/SUPERVISOR_RECONSTRUCTION_GAP_REGISTER.md`, `ai/SUPERVISOR_MVP_INCLUSION_DEFER_MATRIX.md`, and `docs/supervisor-ui-docs/*` reconstruction docs.

- WAITER_PREMIUM_ME_SHARED_PROFILE Prompt 5 complete / authenticated and viewport QA passed (2026-07-18): Waiter Me is now a premium operational profile with verified identity, role-aware accent, explicit On shift/Off shift/Shift issue state, one focused shift action, concise attendance/leave/shift-swap history, branch/account context, and secure sign out. Missing employee linkage is consolidated into one primary notice; employee-dependent queries are suppressed and affected sections render compact truthful unavailable states. Reusable presentation-only profile primitives are safely shared with Cashier and Supervisor while role pages retain query, mutation, permission, and business-rule ownership. Cashier narrow-shell overflow is resolved at 1024x768, abnormal till-without-shift state is warning-first, and Supervisor header/profile density is responsive. Final authenticated Playwright QA passed for linked and unlinked Waiter, Cashier, and Supervisor, including Waiter Floor, Reservations, owned-table menu, order summary, bill/receipt, logout, and return login. Per-role profile/workforce requests occurred once, no console/request failures were recorded, and measured scroll width matched viewport width at 1024x768, 1366x768, 1440x900, and 1920x1080. pnpm 8.15.0, focused profile assertions, web typecheck, lint, production build, and `/api/health` db ok all passed. No API, Prisma, migration, seed, Postman, routing, cache-key, or performance-architecture contract was changed. Report: `ai/WAITER_PREMIUM_ME_SHARED_PROFILE_COMPLETION_REPORT.md`.

- APPLICATION_PERFORMANCE_HARDENING Prompt 4 blocker closed / strict latency targets partially unmet (2026-07-18): shared auth, branch-context, API-client, React Query, and role-surface latency hardening completed across Waiter, Cashier, and Supervisor without redesigning approved workflows. JWT validation now reuses token role/permission claims, `/api/auth/me` reuses request claims and parallelizes membership/employee reads, BranchContextGuard caches/dedupes branch membership checks briefly, Quick PIN avoids a duplicate branch lookup and routes immediately from branch session context, and the web API client now has request IDs, AbortController wiring, empty-response-safe parsing, and bounded 30s timeouts. Waiter/cashier/supervisor mutation handlers no longer await unrelated invalidation groups before clearing pending UI. Cashier Queue, Cashier Receipts, and Supervisor Orders no longer launch list-level per-row payment/receipt fan-outs; selected-row detail reads remain authoritative. Validation passed: pnpm 8.15.0, web typecheck/lint/build, db demo validation, focused Quick PIN tests plus earlier focused auth/branch/orders tests, all 56 Postman JSON files parsed, `/api/health` db ok, and authenticated Playwright smoke at 1440x900 for Waiter/Cashier/Supervisor. Measured residual latency remains from Neon/local DB round trips: warm Quick PIN improved to ~3.8s after cold-ish 9.9s, `/auth/me` reached ~1.46s warm, cashier request count dropped from 101 to 9, waiter table-to-workspace measured 182ms, but item detail/catalog can still exceed warm targets in this environment. Report: `ai/APPLICATION_PERFORMANCE_HARDENING_COMPLETION_REPORT.md`.

- WAITER_MENU_VISIBILITY_UGX_TOTALS Prompt 4 implementation / authenticated QA blocked (2026-07-18): Tapas Downtown menu navigation root cause was missing importer support for browse groups, subgroups, and item assignments; new demo CSVs and importer paths now populate FOOD/DRINKS navigation without hardcoded frontend taxonomy. Legacy seed prices that could reintroduce `UGX 8.5`-style values were updated to realistic whole UGX values and existing seed rows are corrected. Waiter currency display now uses one central formatter with branch currency context and zero-fraction UGX rendering. Order item mutations now write backend-derived/pending snapshots and refresh canonical order data; send returns after the status transition while KDS ticket creation remains best-effort in the background. Backend item writes now reject inactive items, servings, and modifier options. Validation passed for web typecheck/lint/build, focused API order/menu/auth tests, demo validation, Postman JSON parsing, and `/api/health`. Authenticated browser QA did not complete: after removing duplicate API/watch processes, direct API timings remained slow and browser add-item confirmation could remain pending while the DB write had landed, blocking edit/remove QA. Report: `ai/WAITER_MENU_VISIBILITY_UGX_AND_TOTALS_COMPLETION_REPORT.md`.

- WAITER_INSTANT_TABLE_MENU_FLOW complete / authenticated performance QA passed (2026-07-17): waiter-operable table selection now synchronously mounts the full-screen menu workspace with table label, Back to Floor, cached navigation/catalog, search, menu items, and order panel; the previous centered draft-resolution/loading card and post-click table-scoped draft lookup were removed. Floor now prefetches menu navigation/catalog and carries each table's active order summary. Owned active orders render from cache and reconcile canonical detail in the background. Available tables create their authoritative dine-in order in the background with progress/error confined to the order panel; one shared creation promise safely queues an early item selection and prevents duplicate order/add submissions. Reserved seating passes its linked order directly into the same workspace; other-waiter ownership remains blocked. Authenticated Playwright QA measured click-to-shell/menu/pending at 71.1/71.6/71.6 ms and cached click-to-shell/menu/order at 23.2/23.6/23.6 ms, verified menu prefetch, no post-click menu request, no blocking draft lookup, exactly one order create, and one queued item commit with no console/network failures or prohibited copy. Web lint, typecheck, production build, health, JSON validation, and diff checks passed. No DB/Prisma/migration/seed/API/Postman contract change. Report: `ai/WAITER_INSTANT_TABLE_MENU_FLOW_COMPLETION_REPORT.md`.

- WAITER_PREMIUM_MENU_ORDER_ENTRY complete / authenticated and viewport QA passed (2026-07-16): Floor cards no longer expose guest names; Available selection now performs list-first waiter-owned draft resume/create and immediately opens one canonical URL-backed full-screen workspace shared with owned Occupied tables and seated reservations. It consumes existing navigation/catalog/item/serving/modifier contracts, renders API-ordered sections/groups/subgroups, whole-menu search, minimal tiles, validated configurator, exact-line edit/remove, returned subtotal/tax/discount/total, and text-first send/bill/receipt actions. Playwright found and verified fixes for a responsive duplicate-mount race and drawer clipping. Sent-order additions remain blocked; serving edit remains read-only. No backend/Prisma/migration/seed/package/Postman change. Typecheck, lint, build, zero-write demo validation, API health, scans, authenticated QA, and 1366×768/1440×900/1920×1080 checks passed. Report: `ai/WAITER_PREMIUM_MENU_ORDER_ENTRY_COMPLETION_REPORT.md`.
