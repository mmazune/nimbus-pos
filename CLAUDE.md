...
COMPLETE / B5.6 · B5.7 · B6 · B7 STILL GATED.** Backend + schema (one new migration) + seed + docs.
**No frontend file touched** — the Manager accounting UI still renders zero write affordances; that
is now a deliberate build-order gate (Track **B5.7**, not started), not a permission gate.
🔴 **PERMS-2 — the owner decided Manager has full access to everything it is responsible for.**
This REVERSES the 2026-08-20 C-21 cutover's OD-9 read-only resolution for Manager. `packages/db/prisma/seed.ts`'s
...
granted** — verified empty on the isolated QA stack (only Owner/Accountant hold it). Waiter, Cashier
and Supervisor gained **zero** accounting/finance/procurement strings — verified empty via direct SQL
against the isolated stack, and via the seed's own idempotence (identical grant counts on a
second run). Seed idempotence re-verified: 54/54/54 accounting+finance permission rows for
Owner/Manager/Accountant after a second seed run, 0 duplicates.
🔴 **C-25 FIXED** — `LedgerService.getJournal` resolved by `{id, orgId}` alone with no branch
predicate at all; now uses `branchOrOrgScope`, matching `listPostingErrors`/`getPostingError`. Live
e2e-proven: a journal id from branch A now 404s under branch B's header. **BGB3-L3 FIXED in the same
pass** — `listJournals`/`listPostingRuns` used strict `where.branchId=` on nullable columns; now the
same `OR: [{branchId}, {branchId: null}]` predicate.
🔴 **C-26 FIXED** — all six pre-existing `ledger.service.ts` `audit.log(...)` calls
(`JOURNAL_CREATED`, `JOURNAL_REVERSED`, `POSTING_RUN_STARTED`, `POSTING_RUN_FINISHED` ×2,
`POSTING_ERROR_CREATED`) now stamp `metadata.branchId`, so ledger-domain events are finally reachable
through the branch-scoped audit-timeline endpoint (B5-F4). **Historical rows created before this fix
still lack `branchId`** — this batch never touched shared Neon, so the production count of affected
rows could not be measured from this environment and was not backfilled (explicitly out of scope,
matching the B5-F4 precedent of never backfilling history).
🔴 **C-27 — legitimised, not fixed-as-a-bug.** The pre-existing, un-audited M28-era Manager grant
(`pos:accounting:periods:create`/`accounts:create`/`cost-centers:create`) is now correct under
PERMS-2 and is seeded as an explicit, commented entry rather than an accident. Live e2e-proven:
Manager `POST /accounting/periods` → 201 with a resolvable `FISCAL_PERIOD_CREATED` audit row, and
`PATCH .../open`, `.../close`, `.../lock` now all return 200 for Manager (previously 403), each
already emitting its own audit event via the pre-existing `accounting.service.ts`/`bank-rec.service.ts`
audit calls. **Every "Manager performs zero accounting writes" / "READ-ONLY BY PERMISSION" claim in
`ai/ENTERPRISE_B5_1…5_*_COMPLETION_REPORT.md`, `ai/ENTERPRISE_UI_ROADMAP.md`,
`ai/ACCOUNTING_API_VERIFICATION_REPORT.md` and `ai/PERMISSIONS_CUTOVER_COMPLETION_REPORT.md` is now
dated by a supersession banner** added to the top of each file — the history is preserved, not
rewritten, per §19.
🟡 **B5.4-D1 FIXED** — `PostingError` had no resolve/dismiss write path anywhere in the API, for any
role. New migration `20260821200636_b5_4_d1_posting_error_resolution` adds
`resolvedById`/`resolvedAt`/`resolutionNotes` columns; new `PATCH /api/accounting/posting-errors/:id/resolve`
and `.../dismiss` routes, gated on new permission `pos:accounting:posting-errors:resolve` (Owner,
Manager, Accountant only), status-guarded (only an OPEN error may transition; a repeat attempt →
409), and stamping a C-26-correct branch-scoped audit event
(`POSTING_ERROR_RESOLVED`/`POSTING_ERROR_DISMISSED`). ⚠️ The hand-written migration deliberately
does **not** use `prisma migrate dev`'s raw diff — that diff also contained a large amount of
pre-existing, unrelated schema drift (FK drop/recreate cycles, index renames on
sync/flags/training/maintenance tables) that predates this batch; bundling it into this migration
would have silently applied unrelated changes to production on the next deploy, so only the
`posting_errors` columns + FK are in the committed migration file.
🟢 **B5.5-F1 — investigated further, confirmed genuinely dead, NOT implemented.**
`PeriodCloseRunStatus.FAILED`/`.PENDING` remain structurally unreachable: `closeFiscalPeriod()` is
the ONLY code path that ever writes a `PeriodCloseRun` row, it is fully synchronous inside one
`$transaction`, and a repo-wide grep found zero `@Cron` decorators and zero BullMQ/queue processors
anywhere in `apps/api` that reference `PeriodCloseRun`. This is case (a) from the batch brief — no
fake transition was added; a future batch may propose removing the two dead enum members.
**Validated on an isolated local Docker stack** — Postgres `:55480` (`nimbus_b4_qa`); shared Neon was
**never connected to or written**. `packages/db/.env` was temporarily swapped for the Prisma CLI
steps only (migrate + generate + seed) and restored immediately after, SHA-256 verified identical to
the baseline; `apps/api/.env` was **never edited on disk** — isolation was achieved by exporting
`DATABASE_URL`/`DIRECT_DATABASE_URL` directly into the spawning shell (`dotenv` never overrides an
already-set env var), so its SHA-256 is trivially identical throughout. API typecheck (`tsc --noEmit`)
clean; targeted `eslint` (no `--fix`) clean (0 errors) on every touched file, plus 4 **pre-existing**
prettier errors on two untouched describe blocks in `accounting-branch-scoping.e2e-spec.ts`, disclosed
not fixed. Ledger unit suite **53/53** (`ledger.service.spec.ts` + `dto/query-dtos.spec.ts`, both
extended with C-25/C-26/B5.4-D1 coverage). New/extended `accounting-branch-scoping.e2e-spec.ts` —
**43/43** passing live against the isolated stack, covering C-25 cross-branch 404, BGB3-L3 OR-nullable
scoping, C-26 audit-timeline reachability, B5.4-D1 resolve/dismiss/409/cross-branch-404, and PERMS-2
Manager writes (period create/open/close/lock, journal create/reverse, posting-error resolve). See
`ai/BACKEND_GAP_BATCH4_COMPLETION_REPORT.md` for the full validation table including Playwright/newman/
cross-role results. **B5.6 (Configuration), B5.7 (the new accounting write-UI pass) and B7 remain NOT
started — do not begin any of them without explicit owner authorisation.**

**Prior milestone record (superseded above) — ENTERPRISE UI TRACK B5.5 COMPLETE — Manager Accounting Closing surfaces (2026-08-21) — A: B5.5
COMPLETE / B5.6 GATED.** Frontend + docs only; **no backend / schema / migration / seed /
...
goes from 19 live rows to 21** (of 28 total). Unlike every prior B5 phase, this one found a real
permission-boundary leak, not a data-shape defect.
...
permissions cutover never audited or revoked (the cutover only ADDED 15 new AP/AR/bank-rec/finance
read strings and never swept the older M28/M29-era grants). Live-proven: Manager
...
probe `periods:create`/`accounts:create`/`cost-centers:create`, so this went undetected through
B5.1–B5.4. Not fixed here (a seed/permission change is out of scope for a frontend-only phase) —
the Fiscal periods screen discloses the gap by name (`data-accounting-finding="C-27"`) and ships
zero create control regardless.
**Fiscal periods** — list-only (`accounting.controller.ts` declares no `GET .../:id`, the same "no
detail route" shape B5.4 found for Posting runs), a `ManagerStatusPipeline` per row rendering the
real PC-07 lifecycle DRAFT→OPEN→CLOSED→LOCKED with LOCKED a genuine terminal stage — unlike bank
reconciliation's `DISPUTED` exit chip, there is no real side-branch a fiscal period can be diverted
to, so the exit-chip mechanism is reused for exactly one thing: an unrecognised status value,
proven live by a mocked malformed-response Playwright spec, not merely asserted. **Period close
runs** — list-only, showing the fiscal period, closed-by actor (resolved to a real name), and an
income/expense/retained-earnings money tie-out — retained earnings correctly rendering **negative**
(−UGX 3,297,400 on the live fixture) when expense exceeds income, never silently flipped or hidden.
...
creates the resulting run with `status: 'COMPLETED'`, and a refused close throws before any
`PeriodCloseRun` row is created at all — proven live by closing an already-CLOSED period (409, zero
new rows). The status badge still renders all three enum members and fails closed on anything else,
but the screen's footnote discloses FAILED/PENDING have never actually been produced.
Both routes confirmed **organisation-level, live, in the browser**: switching Tapas Downtown →
Rooftop Bar leaves both the 7-row Fiscal periods list and the 1-row Period close runs list
**byte-identical**. Both are PC-06 bare arrays with no server total — labelled "Showing all N",
never a fabricated pager. **PC-07 re-verified live, precisely**: `PATCH .../lock` on a CLOSED
period → 200 `LOCKED`; a subsequent `PATCH .../unlock` → **404** — the route genuinely does not
exist, confirmed on the isolated stack.
The B5.1 dashboard's **Fiscal period card** — all three figures inert since B5.1 — is now wired for
real: `period.current`/`period.open`/`period.closeRuns` all gained a real `drillIn` in
`ACCOUNTING_KPI_BINDINGS`; the card's own fail-closed rendering logic was **not** rewritten — only
the registry's `drillIn` changed.
**Fixtures created live via the API on the isolated stack** (Owner token as the intended actor,
plus deliberately the **Manager token** for the one C-27 verification call), on Tapas Downtown
(which already carried 5 fiscal periods — 3 OPEN, 2 CLOSED — and 0 close runs from
`db:demo:import`): 1 fresh DRAFT period (Owner); 1 previously-OPEN period closed live via
`PATCH .../close`, producing the **one real `PeriodCloseRun`** (COMPLETED, incomeTotal
UGX 3,164,200, expenseTotal UGX 6,461,600, retainedEarningsAmount −UGX 3,297,400, `branchId: null`);
a second close attempt on the same now-CLOSED period → 409 with zero new rows (the live B5.5-F1
proof); 1 previously-CLOSED period locked live via `PATCH .../lock`, confirmed terminal; 1 further
DRAFT period created via the **Manager token itself** (the C-27 proof), with Manager
`PATCH .../open`/`.../close`/`.../lock` on it all returning 403. Final Tapas Downtown counts: **7**
fiscal periods (DRAFT ×2, OPEN ×2, CLOSED ×2, LOCKED ×1 — all four `FiscalPeriodStatus` members
live in one organisation), **1** period close run (COMPLETED). Rooftop Bar confirmed to render the
byte-identical 7-period and 1-close-run lists, live in the browser via the branch switcher and
separately via `curl` under a different `X-Branch-Id` header.
**Validated on an isolated local Docker stack** — Postgres `:55470` (`nimbus_b55_qa`), API `:4081`,
web `:3170`; **shared Neon was never connected to or written** (the isolated API held exactly one
established TCP connection, to its own local Postgres, verified via `lsof`); `apps/api/.env` was
never edited on disk (the isolated database target was supplied by an explicitly constructed
child-process environment, SHA-256 identical before and after); `packages/db/.env` was temporarily
swapped for the three Prisma CLI steps only, restored immediately after, SHA-256 verified
byte-identical to the baseline. Web typecheck + lint (no `--fix`) + production build all pass (2
new pages); **17/17** assertion scripts (`manager-b5-assertions.ts` extended with a new §15 of
B5.5-specific checks — no fabricated pager on either bare-array route, both routes re-confirmed
organisation scope, the fiscal-period pipeline modelled as exactly four linear stages with no
side-branch, fail-closed status helpers exercised against concrete vectors, the C-27 and B5.5-F1
disclosures pinned by name in both the registry and the screens, no interactive control or
write-function name anywhere in the Closing tree); `e2e/manager-accounting/closing.spec.ts` (new,
19 specs incl. two mocked-malformed-response fail-closed tests and two mocked-500 tests) executed
across all 4 viewport projects — **76/76 passed, 0 failed**; the updated
`e2e/manager-accounting/menu-and-read-only.spec.ts` (19→21 row count, Fiscal periods/Period close
runs inserted after Journal entries, inert-row-count 9→7) — **29 passed / 3 skipped** across 4
viewports (the pre-existing "desktop dropdown only at `xl`" skip class, not a B5.5 gap); live manual
QA toured both new pages plus the dashboard's Fiscal period card at 1440×900 and a
...
Configuration) is NOT started — do not begin it without explicit owner authorisation. A future
backend batch will need to address C-25, C-26, B5.4-D1, C-27 and B5.5-F1 before B6 (Settings) can
start.**

**Prior milestone record (superseded above) — ENTERPRISE UI TRACK B5.4 COMPLETE — Manager Accounting core (Journal entries) + Review surfaces
(2026-08-21) — A: B5.4 COMPLETE / B5.5…B5.6 GATED.** Frontend + docs only; **no backend / schema /
migration / seed / permission / DTO / Postman change**. The four Accounting/Review menu rows B5.1
shipped as honest not-yet placeholders — Journal entries, Posting runs, Posting errors, Audit trail —
are now real surfaces. **The Accounting menu goes from 15 live rows to 19** (of 28 total). Manager accounting stays **read-only by permission** — the same 15 read strings, zero
writes (PC-01/OD-9, re-verified live: journal post/reverse and posting replay all 403 for Manager); the
no-write-affordance guard was extended over the new tree, never relaxed.
⚠️ **SCOPE CORRECTION, read before touching B5.5/B5.6:** an operator brief for this phase described
fiscal periods (open/close/lock), period close runs, posting source maps and tax configuration as B5.4
deliverables. They are not — `lib/accounting/menu.ts`'s own `ACCOUNTING_SUBPHASES` tags, set in B5.1
and never changed since, assign Fiscal periods/Period close runs to **B5.5 (Closing)** and Chart of
accounts/Cost centres/Posting source maps/Tax configuration to **B5.6 (Configuration)** — matching the
roadmap's own B5.4/B5.5 table rows. Per CLAUDE.md §19, the tags won and neither surface was built in
this phase; confirming the correctness of that call, **none of the four real B5.4 surfaces turned out
to be organisation-level** (all four registry entries are `scope: "branch"`), so the org-level-labelling
ruling this phase's brief anticipated never actually applied to anything built here.
...
independently-computed figures (the journal's own stored totals, and a fresh client-side sum over its
lines — live-proven to agree at UGX 620,000 on a manually-created fixture and at UGX 5,574,000 /
UGX 8,223,600 across two branches' full 9-row / 8-row pages), and reversal linkage (`Reverses` /
`Reversed by`, navigable both directions). **Posting runs** — list only (no detail route exists on this
entity), a server-total paginated list with **no filter of any kind** — the endpoint accepts none, not
even status. **Posting errors** — list + detail (status filter OPEN/RESOLVED/DISMISSED), the raised
`code`/`message`/`details` JSON and its owning posting run. **Audit trail** — list only
(`GET /api/audit/timeline`, the reused BG2 endpoint), scoped to three curated entity types verified
against `ledger.service.ts`'s own source (`JournalEntry`, `PostingRun`, `PostingError`), not the
org-wide feed every other domain also writes into.
🔴 **Two new findings, both live-proven, neither implemented (out of scope for a frontend-only
phase):** **C-25** — `getJournal` resolves by `{id, orgId}` alone, with **no branch predicate at all**,
unlike every other accounting detail route and unlike `listJournals` itself; a journal id from one
branch's list stays readable by the same id under a different branch's header. Mitigated client-side:
`isJournalReadableInBranch()` compares the fetched journal's `branchId` against the active branch and
refuses to render a mismatch (the same MP0-12 fail-safe Track B4 used for cross-branch report runs).
...
**never `metadata.branchId`** — batch 3's B5-F4 fix made the audit-timeline endpoint's default read
unconditionally AND `metadata.branchId = X-Branch-Id`, so this rail can **structurally never** surface
a ledger-domain event. Proven live: 8 fresh journal/reversal/posting-run/posting-error events were
created via this phase's own fixtures, and every one of the resulting `AuditLog` rows' `branchId` is
`NULL` (direct `psql` query against the isolated database — not stale seed data, events created minutes
earlier through the live API). The Audit trail screen's empty state and footnote **name the C-26 gap
explicitly** rather than reading as "nothing happened". Also disclosed: **B5.4-D1** — there is
genuinely **no resolve/dismiss endpoint for `PostingError` anywhere in the API**, for any role
(confirmed by grepping every controller — only `pos:accounting:posting-errors:read` is seeded, no
`:resolve`/`:dismiss`/`:write` string exists at all), so the Posting errors detail states this plainly
instead of reusing `AccountingReadOnlyCard`'s "an Owner or Accountant performs this" copy, which would
be false here.
The B5.1 dashboard's **General ledger card** — shipped in B5.1 with all three figures inert
(`noDrillInReason: NOT_YET("B5.4", …)`) — now links for real: `ledger.journals`, `ledger.postingRuns`
and `ledger.postingErrors` all gained a real `drillIn` in `ACCOUNTING_KPI_BINDINGS`. The **Fiscal
period** card is correctly untouched — it stays `NOT_YET("B5.5", …)`, since wiring it now would be
unauthorised B5.5 work.
**Fixtures created live via the API (Owner token — Manager holds no accounting write) on the isolated
stack**, on Tapas Downtown (which already carried 5 real journals from `db:demo:import`): 2 manual
balanced journals (`JNL-000044`, and `JNL-000045` → reversed into `JNL-000046`), plus a
`POST /accounting/posting/replay` pair (one with a real active `sourceKey` → SUCCEEDED run + an
auto-created journal; one with an unknown `sourceKey` → FAILED run + one OPEN posting error). Final
Tapas Downtown counts: **9** journals, **2** posting runs, **1** posting error. Rooftop Bar confirmed
to carry its own unrelated **8** journals and **0** posting runs/errors, proving a branch switch
re-scopes to genuinely different data.
**Validated on an isolated local Docker stack** — Postgres `:55460` (`nimbus_b54_qa`), API `:4071`,
web `:3160`; **shared Neon was never connected to or written**; `apps/api/.env` was never edited on
disk (SHA-256 identical throughout); `packages/db/.env` was temporarily swapped for the three Prisma
CLI steps only (the documented, unavoidable exception — the Prisma CLI resolves `DATABASE_URL` from
the schema-adjacent `.env` regardless of an inline override or a `dotenv-cli -o` override, confirmed on
this Prisma version) and restored immediately after, SHA-256 verified byte-identical to the baseline.
Web typecheck + lint (no `--fix`) + production build all pass (4 new pages); **17/17** assertion
scripts (`manager-b5-assertions.ts` extended with a new §14 of B5.4-specific checks — separate
debit/credit columns, the balance tie-out helpers exercised against concrete vectors, the C-25 and C-26
disclosures pinned by name, `audit.timeline`'s stale `scope: "organization"` INVERTED to `"branch"`);
`e2e/manager-accounting/core-and-review.spec.ts` (new, 13 specs) and the updated
`e2e/manager-accounting/menu-and-read-only.spec.ts` (15→19 row count) executed across all 4 viewports;
`e2e/manager-shell/` regression executed; live manual QA toured all 8 new/changed pages/states across
both Tapas Downtown (populated, incl. the C-26 audit-trail gap) and a branch switch to Rooftop Bar;
zero console errors; `/api/health` → ok throughout; `git diff --check` clean. See
`ai/ENTERPRISE_B5_4_ACCOUNTING_CORE_COMPLETION_REPORT.md` for the full validation table (exact
pass/fail counts) and the tie-out figures. **B5.5 (Closing) and the remainder of B5.6 (Configuration)
are NOT started — do not begin either without explicit owner authorisation.**

**Prior milestone record (superseded above) — ENTERPRISE UI TRACK B5.3 COMPLETE — Manager Accounting Bank reconciliation surfaces (2026-08-21) —
...
⚠️ **PERMS-2 correction, read before touching any Track B5.1–B5.5 boundary paragraph below
(backend gap batch 4, 2026-08-21):** every claim below that "Manager accounting is READ-ONLY BY
PERMISSION", holds "zero writes", or is "5/5 representative writes → 403" described the permission
state **as it existed when that phase shipped**. It no longer describes current permission state —
the owner has since decided Manager has full access to everything it is responsible for, and PERMS-2
granted Manager the full 36-string `C21_ACCOUNTING_FINANCE_ALL` set plus the full M28/M29 set,
matching Accountant/Owner (also resolving **C-27**, **PC-01**, and **PC-02**). **The FRONTEND
no-write-affordance rules below remain in force exactly as written** — do not add a write control,
`useMutation` hook, `<Button>`/`onClick=`, or `<form>` anywhere in the Manager accounting tree without
explicit authorization for Track **B5.7** (the new "accounting write pass" phase, not started; see
`ai/ENTERPRISE_UI_ROADMAP.md`). The gate on the frontend is now a deliberate build-order decision, not
a permission gate — do not use "the permission exists now" as a reason to add a write control. **C-25**
(`getJournal` branch scoping) and **C-26** (ledger audit `branchId` stamping) are FIXED at the backend
— the frontend's `isJournalReadableInBranch()` fail-safe and the Audit trail C-26 disclosure copy stay
in place as defense-in-depth per the existing rule below, now describing a fixed-but-still-undisclosed-
to-the-UI backend state. **B5.4-D1** (posting-error resolve/dismiss) is FIXED — a real endpoint exists
now, gated on new permission `pos:accounting:posting-errors:resolve` (Owner/Manager/Accountant only);
the frontend still renders no such control until B5.7. See
...
- Manager reconstruction **M-P1, Track B1 (top-nav shell), Track B2 (Overview dashboard), Track B3
  ...
  **Only Settings still renders the honest foundation screen.** Do not add an approval **decision** control to Overview, render a KPI that is not in
  ...
  B5.2 + B5.3 + B5.4 + B5.5 (Accounting) are now COMPLETE**, plan any further sub-phase from
  `ai/ENTERPRISE_UI_ROADMAP.md` Track B, not from `ai/MANAGER_RECONSTRUCTION_ROADMAP.md`. **B5.6,
  B6 (Settings) and B7 remain NOT started — do not begin any of them without explicit
  authorization.**
...
- **Track B5.3 boundaries — do not cross without explicit authorization.** **Reconciliation is
  ...
  a LIST pager to either file. **B5.1, B5.2, B5.3, B5.4 and B5.5 are COMPLETE. B5.6 (Configuration
  + the remainder of Reporting) is NOT started — do not begin it without explicit owner
  authorisation.**
- **Track B5.4 boundaries — do not cross without explicit authorization.** **Journal entries, Posting
  runs, Posting errors and Audit trail are READ-ONLY** — no post/reverse/replay/resolve/dismiss
  control anywhere, not even disabled; Manager holds `journals:read`/`posting-runs:read`/
  `posting-errors:read`/`audit:read` but none of `journals:create`/`:reverse`, `posting:replay`
  (re-verified live 403). Do not add fiscal periods, period close runs, posting source maps or tax
  configuration as part of any future pass calling itself "B5.4" — the menu tree's own
  `ACCOUNTING_SUBPHASES` tags (unchanged since B5.1) assign those to **B5.5**/**B5.6**, and B5.4's own
  completion report records that an operator brief conflated them; do not repeat that conflation. Do
  not build a trial balance, general ledger statement, or any other financial-statement surface — none
  exists on this backend (NG-07 → C-11); "General ledger" in this app **is** the Journal entries list.
  Do not relax the journal lines table to a single signed amount column — Debit and Credit are always
  two separate columns. Do not remove the Balance tie-out card's independent line-sum computation
  (`sumJournalLineAmounts`) in favour of only showing the journal's own stored total — the point is two
  independently-computed figures, not one trusted field. Do not remove or weaken
  `isJournalReadableInBranch()` on the journal detail screen — it is the C-25 fail-safe for a real,
  live-proven backend gap (`getJournal` has no branch predicate at all); do not "fix" it by widening it
  to accept any branch. Do not remove the C-26 disclosure from `AuditTrailScreen.tsx` or revert its
  empty-state copy to the generic "no X match the current filter" wording — the rail is verified-empty
  by a real backend defect (`ledger.service.ts`'s `audit.log` calls never stamp `metadata.branchId`),
  not by an empty history, and the generic copy would be false. Do not add a status filter to Posting
  runs — the endpoint accepts none, and a client-side filter over one page of a real paginated list
  would misrepresent completeness (this is NOT the PC-06 bare-array precedent, where the array is
  already the complete result set). Do not reuse `AccountingReadOnlyCard`'s "an Owner or Accountant
  performs this" copy on the Posting errors detail — B5.4-D1 established that no role can resolve or
  dismiss a posting error through this API, so that copy would be false there specifically. Note for
  whoever starts B5.6: **C-23** — the M33 GL Postman collection cannot run (pre-existing, proven by
  backend gap batch 2), so B5.4's journals surface shipped without Postman verification; do not
  silently "fix" the collection as part of a later phase without separate authorisation.
- **Track B5.5 boundaries — do not cross without explicit authorization.** **Fiscal periods and
  Period close runs are READ-ONLY** for the actions the owner ruling covers — no open/close/lock
  control anywhere, not even disabled; `PATCH .../open`/`.../close`/`.../lock` all re-verified 403
  for Manager. ⚠️ **C-27**: Manager's token additionally holds `pos:accounting:periods:create` (a
  pre-existing, never-revoked write) — **do not add a create control just because the permission
  technically allows one**; the owner's read-first ruling governs the UI regardless of what one
  stray permission string permits, and fixing the permission itself requires separate seed
  authorisation (see the C-27 correction note above). Do not add fiscal periods/period-close-runs
  detail routes or `?periodId=`/`?runId=` URL state — neither entity has a `GET .../:id` endpoint
  (confirmed against `accounting.controller.ts`/`bank-rec.controller.ts` directly), the same
  "list-only, no detail route" shape B5.4 established for Posting runs. Do not build a period-close
  readiness/checklist view — no endpoint backs one. Do not relax `fiscalPeriodPipelineIndex()` to
  treat an unrecognised status as stage 0 (DRAFT) — an unreadable value must render "Status
  unavailable", never a real stage, because stage 0 is indistinguishable from a genuine confirmed
  DRAFT period. Do not remove the B5.5-F1 disclosure from `PeriodCloseRunsScreen.tsx` — `.../close`
  always creates the run COMPLETED or refuses before any row exists, so `FAILED`/`PENDING` have
  never actually been produced by this backend, and implying otherwise would be false. Do not bind
  a pager to either B5.5 route — both are PC-06 bare arrays with no server total. Do not relabel
  either surface as branch data — both are organisation-level by design (batch 2): `FiscalPeriod`
  has no `branch_id` column at all, and `PeriodCloseRun.branchId` is nullable and never stamped by
  the close path. **B5.1–B5.5 are COMPLETE. B5.6 (Configuration + the remainder of Reporting) is
  NOT started — do not begin it without explicit owner authorisation. A future backend batch will
...
- **The permissions cutover is complete and must not be undone.** Do not remove any of the **36**
  ...
  **prefix** match — that is exactly how the original count missed bank-rec's 11 strings and
  ...
  hand Manager a write.~~ **SUPERSEDED by PERMS-2 (backend gap batch 4, 2026-08-21):** the owner
  reversed OD-9 — Manager now holds the full 36-string set, including `procurement:advisory:read`
  with its write half. **Do not restore `pos:hr:compensation:read` to Manager** (FU-1) — Owner and
  ...
  ⚠️ **C-27 correction (Track B5.5, 2026-08-21):** "Manager READ-ONLY, 15 strings" above describes
  ONLY what the C-21 cutover itself added — it was never a claim that Manager holds zero accounting
  writes overall. Track B5.5 found that a **pre-existing, M28-era** seed grant
  (`pos:accounting:periods:create`, plus `accounts:create`/`cost-centers:create`) predates the
  cutover and was never audited or revoked by it. Live-proven: Manager `POST /accounting/periods` →
  201. **Do not treat "Manager READ-ONLY" as literally true of every accounting permission string
  Manager holds** — it is true of the 36 cutover-added strings and of `open`/`close`/`lock`
  (re-verified 403), not of these three older `:create` strings. **Do not fix C-27 without a
  separate, explicitly authorised seed change** — it follows the same rule as C-21/C-22.
...
- **B5 boundaries set by B0 (`ai/ACCOUNTING_API_VERIFICATION_REPORT.md`, verdict upgraded to 🟢
  ...
  **`DRAFT → OPEN → CLOSED → LOCKED`** with no unlock route (**PC-07**). ⚠️ note **C-23**, the M33
  collection cannot run, so the journals surface has no Postman verification. ~~Ship journals
  **read-only** for Manager — the guides are live-verified correct (`journals:create` / `reverse` /
  `posting:replay` → 403). **Manager still holds NO accounting write** (**PC-01**) and is
  deliberately denied `procurement:advisory:read` (**PC-02**) — B5 must request those explicitly.~~
  **SUPERSEDED by PERMS-2 (backend gap batch 4, 2026-08-21):** Manager now holds full journal
  create/reverse/replay and `procurement:advisory:read` at the backend — PC-01 and PC-02 are
  resolved by owner decision. The **frontend** journals surface is still deliberately read-only
  (Track B5.7, not started) — see the §12 top-of-section correction note.
