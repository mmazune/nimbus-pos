...
3. **`PATCH /api/branches/:id`** — route does not exist (404). **M-P6's branch profile is
   read-only.**
4. **`GET /api/hr/employees`** — returns `compensationProfile{baseAmount, salaryBasis, allowances,
   deductions}` on **every** row, plus `dateOfBirth`, `address`, and private HR `notes`, and is
   **org-scoped with no branch filter**. This violates the locked *"compensation never fetched"*
   constraint at the wire level; a frontend whitelist is not sufficient.
...
| **Staff** | GET | `/api/hr/employees` | `HrController` | `pos:hr:employees:read` | Branch-scoped | Medium | Read | Yes | Lists employee records. | 🔴 **200 but VIOLATES the locked scope.** `hr.controller.ts:46-47`, perm matches. **`hr.service.ts:252` unconditionally `include: { position: true, compensationProfile: true }`** — all 40 live rows returned `compensationProfile{salaryBasis, baseAmount (e.g. "2800000"), currency, allowances, deductions}`. Full field set also includes PII: `email, phone, address, dateOfBirth, emergencyContactName, emergencyContactPhone, notes (private HR notes), metadata`. Safe: `id, orgId, branchId, employeeCode, status, firstName, middleName, lastName, employmentType, hireDate, positionId, position{...}, userId, createdAt, updatedAt`. A frontend whitelist does not stop the wire transfer — M-P4 needs an **allow-list projection at the API-client boundary** (MP0-01). ⚠ Also **ORG-scoped** — `hr.service.ts:232` is `where = { orgId }` with **no** branch filter; live rows spanned 5 branches. `?branchId=` → **400** (not whitelisted); no branch filter exists (MP0-06). ⚠ `take` **unbounded** (`@IsNumberString` only); `?take=500` accepted. |
| **Staff** | POST | `/api/hr/employees` | `HrController` | `pos:hr:employees:create` | Branch-scoped | High (PII) | Write | Yes | Creates staff user record. Comp fields must be omitted from UI. | 🟢 not exercised (mutation) — `hr.controller.ts:32-33`, perm matches. |
| **Staff** | PATCH | `/api/hr/employees/:id` | `HrController` | `pos:hr:employees:update` | Branch-scoped | High | Write | Yes | Updates staff profile (excludes compensation fields). | 🟢 not exercised (mutation) — `hr.controller.ts:60-61`, perm matches. ⚠ `GET /api/hr/employees/:id` (`:53-54`, same read perm) additionally returns **`contracts[]`** with `salaryBasis` + `salaryAmount` — M-P4 should avoid the detail route when the list row suffices (MP0-01). |
...
| **Reports** | POST | `/api/reports/export` | `ReportsController` | `pos:reports:exports:read` | Branch-scoped | Medium | Write | Yes | Packages report run payload into an export file. | 🟡 **201 — EXECUTED LIVE (CSV + PDF).** `reports.controller.ts:607-609`. **`pos:reports:exports:read` on a WRITE route CONFIRMED verbatim** — a read permission gating a route that creates an `ExportArtifact` and writes a file. Recorded as a **backend guard defect** (MP0-13); no Manager impact (all export perms held); **not fixed**. DTO `CreateExportDto = {reportRunId!: string, format!: CSV\|PDF}`. 🔴 **The PDF is NOT a PDF** — `reports.service.ts:2056` calls `generateTextPdf()`, a plain-text builder, while `:1997-1999` stamps `mimeType: 'application/pdf'` + `.pdf`. The downloaded 790-byte artifact is `Unicode text, UTF-8` with **no `%PDF-` header**, yet `status: READY`. **M-P5 must offer CSV only** (MP0-03). ⚠ The CSV is the **summary only** (11 metric rows) despite `rowCount: 219`. Failure modes truthful: unknown run → **404**; non-COMPLETED run → **400**; generator throw → artifact `FAILED` + `failureReason`. ⚠ Lookup is `{id, orgId}` — **no branchId** (MP0-12). |
| **Reports** | GET | `/api/reports/exports/:id/download` | `ReportsController` | `pos:reports:exports:download` | Branch-scoped | Medium | Read | Yes | File download stream for generated reports. | 🟢 **200** — `reports.controller.ts:622-624`, perm matches. CSV: `Content-Type: text/csv`, 254 bytes, well-formed. PDF: `Content-Type: application/pdf`, 790 bytes, **plain text** (see the export row). Unknown artifact → **404** `Export artifact not found`. |
...
`exports.controller.ts` (prefix `exports`): `GET /api/exports/:id`, `GET /api/exports/:id/download`. Distinct from `GET /api/reports/exports/:id/download` which the matrix does carry. **MP0-03 (the plain-text-stamped-as-PDF finding) applies to whatever artifact these serve — verify before use.**
