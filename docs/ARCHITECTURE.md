...
M24 adds workforce tracking on top of M23's HR Core: daily clock-in/out with late-detection, leave request workflows, shift swap negotiation, and configurable attendance policies per organization/branch.

### Models

- **AttendanceRecord** — One record per employee per day. Tracks clockInAt, clockOutAt, status (CLOCKED_IN → CLOCKED_OUT or LATE), with unique constraint on `[employeeId, attendanceDate]`.
- **LeaveRequest** — Leave request from an employee: type, date range, status machine (PENDING → APPROVED/REJECTED/CANCELLED), reviewed-by link.
- **ShiftSwapRequest** — Request from one employee to swap a shift with another: requester, target, shiftDate, approval status.
- **AttendancePolicy** — Configurable policy per org/branch: grace minutes, auto-late threshold, self clock-out fix toggle.
...
- `GET /api/hr/attendance` — List attendance records (filterable by employee, status, date range)
- `POST /api/hr/leave` — Create leave request
- `GET /api/hr/leave` — List leave requests (filterable by employee, status, type)
- `PATCH /api/hr/leave/:id/review` — Approve or reject a leave request
- `POST /api/hr/shift-swaps` — Create shift swap request
- `GET /api/hr/shift-swaps` — List shift swap requests (filterable by employee, status)
- `PATCH /api/hr/shift-swaps/:id/approve` — Approve or reject a shift swap
...
| Permission                       | Description                        |
| -------------------------------- | ---------------------------------- |
| pos:hr:attendance:clock          | Clock in/out employees             |
| pos:hr:attendance:read           | List attendance records            |
| pos:hr:leave:create              | Create leave requests              |
| pos:hr:leave:read                | List leave requests                |
| pos:hr:leave:review              | Approve/reject leave requests      |
| pos:hr:shift-swaps:create        | Create shift swap requests         |
| pos:hr:shift-swaps:read          | List shift swap requests           |
| pos:hr:shift-swaps:approve       | Approve/reject shift swaps         |
| pos:hr:attendance-policy:read    | List attendance policies           |
| pos:hr:attendance-policy:create  | Create attendance policies         |
| pos:hr:attendance-policy:update  | Update attendance policies         |

### M24 Audit Events

| Action                         | Trigger                                     |
| ------------------------------ | ------------------------------------------- |
| ATTENDANCE_CLOCK_IN            | Employee clocked in                         |
| ATTENDANCE_CLOCK_OUT           | Employee clocked out                        |
| LEAVE_REQUEST_CREATED          | Leave request submitted                     |
| LEAVE_REQUEST_REVIEWED         | Leave request approved or rejected          |
| SHIFT_SWAP_CREATED             | Shift swap request submitted                |
| SHIFT_SWAP_REVIEWED            | Shift swap approved or rejected             |
| ATTENDANCE_POLICY_CREATED      | Attendance policy created                   |
