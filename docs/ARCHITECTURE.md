...
M24 extends the HR layer with time-tracking and scheduling operations: clock in/out records, leave requests with a review workflow, shift swap approvals between employees, and configurable attendance policies.

### Models

- **AttendanceRecord** — Tracks clock-in and clock-out events per employee per branch. Records late status and calculates duration. Clock toggle: a second `POST /clock` closes the open record.
- **LeaveRequest** — Employee request for time off. Lifecycle: PENDING → APPROVED / REJECTED (via manager review). Tracks leave type, date range, and reason.
- **ShiftSwap** — Request to swap a shift between two employees. Lifecycle: PENDING → APPROVED / REJECTED. Requires target employee confirmation.
- **AttendancePolicy** — Configurable rules per branch (late threshold, overtime rules, grace period, etc.).
...
- `POST /api/hr/attendance/clock` — Clock in or out (toggle)
- `GET /api/hr/attendance` — List attendance records (filterable by employeeId/date)
- `POST /api/hr/leave` — Request leave
- `GET /api/hr/leave` — List leave requests
- `PATCH /api/hr/leave/:id/review` — Approve or reject leave request
- `POST /api/hr/shift-swaps` — Request shift swap
- `GET /api/hr/shift-swaps` — List shift swap requests
- `PATCH /api/hr/shift-swaps/:id/approve` — Approve or reject shift swap
- `GET /api/hr/attendance/policies` — List attendance policies
- `POST /api/hr/attendance/policies` — Create attendance policy
- `PATCH /api/hr/attendance/policies/:id` — Update attendance policy

### M24 Permissions

| Permission                        | Description                            |
| --------------------------------- | -------------------------------------- |
| pos:hr:attendance:read            | List and view attendance records       |
| pos:hr:attendance:clock           | Clock in and out                       |
| pos:hr:leave:read                 | List and view leave requests           |
| pos:hr:leave:request              | Submit leave requests                  |
| pos:hr:leave:review               | Approve or reject leave requests       |
| pos:hr:shift-swaps:read           | List and view shift swap requests      |
| pos:hr:shift-swaps:request        | Submit shift swap requests             |
| pos:hr:shift-swaps:approve        | Approve or reject shift swaps          |
| pos:hr:attendance-policies:read   | List and view attendance policies      |
| pos:hr:attendance-policies:manage | Create and update attendance policies  |
| pos:hr:attendance:admin           | Admin override for attendance records  |
...
M25 introduces workforce scheduling: shift templates define reusable time blocks; schedules group assignments for a date range; the roster view shows all scheduled shifts in a given week; coverage rules define required staffing levels and generate gap alerts.

### Models

- **ShiftTemplate** — Reusable template for a time slot (e.g., WEEKDAY-AM 07:00–15:00). Unique `code` per branch. Tracks recurrence patterns (daysOfWeek), color, break duration, and max headcount.
- **Schedule** — A planned roster covering a date range (`startsOn`/`endsOn`). Progresses through DRAFT → PUBLISHED → ARCHIVED via explicit PATCH actions. Auto-generates version number on publish.
- **ScheduleAssignment** — A single shift assignment: one employee, one date, one shift template, within a schedule.
- **CoverageRule** — Business rule specifying minimum headcount for a time window (optionally scoped to a role or position). Generates gap alerts when the roster does not meet the rule.
...
All under `/api/workforce`, protected by JwtAuthGuard + PermissionGuard + BranchContextGuard.

- `POST /api/workforce/templates` — Create shift template
- `GET /api/workforce/templates` — List shift templates (filterable by active)
- `POST /api/workforce/schedules` — Create schedule with optional assignments
- `GET /api/workforce/schedules` — List schedules (filterable by status)
- `GET /api/workforce/schedules/:id` — Get schedule with full assignments + templates
- `PATCH /api/workforce/schedules/:id/publish` — Publish a DRAFT schedule
- `PATCH /api/workforce/schedules/:id/archive` — Archive a PUBLISHED schedule
- `GET /api/workforce/roster` — Flattened roster view with date range filter
- `POST /api/workforce/coverage-rules` — Create coverage rule
- `GET /api/workforce/coverage-rules` — List coverage rules
- `GET /api/workforce/coverage-gaps` — Compute gaps between rules and actual roster

### M25 Permissions

| Permission                          | Description                              |
| ----------------------------------- | ---------------------------------------- |
| pos:workforce:templates:read        | List and view shift templates            |
| pos:workforce:templates:create      | Create shift templates                   |
| pos:workforce:schedules:read        | List and view schedules and roster       |
| pos:workforce:schedules:create      | Create schedules with assignments        |
| pos:workforce:schedules:publish     | Publish or archive schedules             |
| pos:workforce:coverage-rules:read   | List and view coverage rules and gaps    |
| pos:workforce:coverage-rules:create | Create coverage rules                    |
